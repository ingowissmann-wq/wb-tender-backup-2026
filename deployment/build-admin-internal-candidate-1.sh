...
CANDIDATE='wb-admin:internal-recovery-candidate.1'
API='wb-tender-production-api'
CONTEXT="$(mktemp -d /srv/wb-tender-recovery/admin-runtime-1.XXXXXXXX)"

cleanup() {
  case "$CONTEXT" in
    /srv/wb-tender-recovery/admin-runtime-1.*)
...
docker build \
  ...
  --build-arg 'RELEASE_VERSION=20260902-admin-internal-recovery.1' \
  --tag "$CANDIDATE" \
  "$CONTEXT"
...
printf '%s\n' 'WB_BUILD_ADMIN_INTERNAL_CANDIDATE_1=SUCCESS'
printf 'candidate=%s\n' "$CANDIDATE"
printf 'candidate_id=%s\n' "$CANDIDATE_ID"
printf 'source_commit=%s\n' "$SOURCE_COMMIT"
printf '%s\n' 'admin_runtime_dependencies=complete'
