...
for (( index=${#applied[@]}-1; index>=0; index-- )); do
  name=${applied[$index]}
  case "$name" in
    177_management_submission_dispatch.sql)
      psql "$url" -v ON_ERROR_STOP=1 -f migrations/177_management_submission_dispatch.down.sql ;;
