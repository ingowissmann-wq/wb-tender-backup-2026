...
BASE_IMAGE='wb-tender:global-selected-lot-region-candidate.15'
BASE_ID='sha256:9bf6560d1f69359c1f9f660c546f0b5b4fc842c92d4f6c8b234d768b6755df90'
CANDIDATE='wb-tender:global-selected-lot-region-candidate.16'
CONTEXT="$(mktemp -d /srv/wb-tender-recovery/region-candidate-16.XXXXXXXX)"

cleanup_build(){
  case "$CONTEXT" in
    /srv/wb-tender-recovery/region-candidate-16.*)
...
printf '%s\n' 'WB_BUILD_TEST_AND_ROLLOUT_GLOBAL_REGION_16=STARTED'
test -d "$REPO/.git"
...
if docker image inspect "$CANDIDATE" >/dev/null 2>&1; then
  ...
  timeout 900s docker build --progress=plain --pull=false --no-cache --network=none \
    ...
    --build-arg 'RELEASE_VERSION=20260902-global-selected-lot-region.16' \
...
ROLLBACK_TAG="wb-tender:rollback-global-region-16-${DEPLOYMENT_ID}"
ROLLOUT_OVERRIDE="${BACKUP_DIR}/candidate-16-${DEPLOYMENT_ID}.override.yml"
ROLLBACK_OVERRIDE="${BACKUP_DIR}/rollback-16-${DEPLOYMENT_ID}.override.yml"
...
printf '%s\n' 'WB_PRIVILEGED_GLOBAL_REGION_ROLLOUT_16=SUCCESS'
printf 'production_image=%s\n' "${CANDIDATE_ID}"
printf 'source_commit=%s\n' "${SOURCE_COMMIT}"
printf '%s\n' 'cleaning=128|128|0'
