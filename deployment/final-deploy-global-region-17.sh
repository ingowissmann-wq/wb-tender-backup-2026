...
printf '%s\n' 'WB_FINAL_GLOBAL_REGION_DEPLOY_17=STARTED'

CANDIDATE='wb-tender:global-selected-lot-region-candidate.17'
CANDIDATE_ID='sha256:30d64f6334519b095f4af837380ac7b56df6ff0c90fb3652a0c100f3528335e3'
...
CLASSIFICATION_GAPS="$(db_query "
  ...
  JOIN tender.current_scoped_region_evaluations evaluation
   ...
   AND evaluation.company_id=selection.company_id
   AND evaluation.canonical_service=selection.canonical_service
  WHERE evaluation.classification IS NULL
     OR btrim(evaluation.classification)=''
     OR evaluation.matching_status IS NULL
     OR btrim(evaluation.matching_status)='';
" | tr -d '[:space:]')"
test "${CLASSIFICATION_GAPS}" = '0'
printf 'current_classification_gaps=%s\n' "${CLASSIFICATION_GAPS}"

REVIEW_HISTORY_GATE="$(db_query "
  WITH expected(canonical_service,classification,matching_status) AS(
    VALUES ('cleaning','MULTI_REGION_REVIEW','REGION_REVIEW_REQUIRED'),
           ('security','MULTI_REGION_REVIEW','REGION_REVIEW_REQUIRED'),
           ('facility_management','REGION_UNRESOLVED','REGION_REVIEW_REQUIRED'),
           ('sicherheitstechnik','REGION_UNRESOLVED','REGION_REVIEW_REQUIRED')
  )
  SELECT count(*)
  FROM expected
  WHERE EXISTS(
    SELECT 1
    FROM tender.region_evaluations historical
    JOIN tender.tender_lot_selections selection
      ON selection.tender_id=historical.tender_id
     AND selection.company_id=historical.company_id
     AND selection.canonical_service=historical.canonical_service
    WHERE historical.lot_id IS NULL
      AND historical.canonical_service=expected.canonical_service
      AND historical.classification=expected.classification
      AND historical.matching_status=expected.matching_status
  );
" | tr -d '[:space:]')"
test "${REVIEW_HISTORY_GATE}" = '4'
printf 'historical_review_classes_preserved=%s|4\n' "${REVIEW_HISTORY_GATE}"

printf '%s\n' '===== CURRENT EXACT CLASSIFICATION DISTRIBUTION ====='
db_query "
  SELECT selection.canonical_service,evaluation.classification,
         evaluation.matching_status,count(*)::integer
  FROM tender.tender_lot_selections selection
  JOIN tender.current_scoped_region_evaluations evaluation
    ON evaluation.id=selection.region_evaluation_id
   AND evaluation.tender_id=selection.tender_id
   ...
   AND evaluation.company_id=selection.company_id
   AND evaluation.canonical_service=selection.canonical_service
  GROUP BY selection.canonical_service,evaluation.classification,
           evaluation.matching_status
  ORDER BY 1,2,3;
"
...
printf '%s\n' 'WB_FINAL_GLOBAL_REGION_DEPLOY_17=SUCCESS'
printf 'production_image=%s\n' "${CANDIDATE_ID}"
printf 'source_commit=%s\n' "${SOURCE_COMMIT}"
printf '%s\n' 'cleaning=128|128|0'
