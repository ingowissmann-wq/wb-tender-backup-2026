...
set -Eeuo pipefail
umask 077
required=(SOURCE_COMMIT RELEASE_IMAGE POSTGRES_IMAGE BROWSER_IMAGE SOURCE_DUMP SOURCE_DUMP_SHA256 SOURCE_DUMP_ENCRYPTION_KEY_FILE EVIDENCE_FILE WIKOS_REAL_TEST_RESULT_FILE)
...
set +o pipefail
toc=$(decrypt_archive | pg_restore -l)
toc_status=$?
set -o pipefail
[[ "$toc_status" -eq 0 ]] || { echo "encrypted source archive catalog is unreadable" >&2; exit 65; }
...
cleanup() {
  docker compose -p "$project" -f "$REHEARSAL_COMPOSE_FILE" down -v --remove-orphans >/dev/null 2>&1 || true
...
docker compose -p "$project" -f "$REHEARSAL_COMPOSE_FILE" exec -T db psql -U postgres -At -d wb_rehearsal -c "SELECT count(*) FROM tender.release_migrations WHERE name IN ('155_autopilot_overview_latest_lookup.sql','156_approved_tender_commercial_plans.sql','157_release_auth_and_commercial_enforcement.sql')" | grep -qx 0
