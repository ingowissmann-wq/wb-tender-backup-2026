...
rollback() {
  trap - ERR
  docker compose -p "$project" -f "$compose" exec -T db psql -U postgres -v ON_ERROR_STOP=1 -d wb_rehearsal < migrations/159_runtime_request_scope.down.sql
