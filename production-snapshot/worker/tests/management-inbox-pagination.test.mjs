...
test("management inbox resolves current relevance by active tender and indexed company scope before DB pagination",()=>{
  ...
  assert.doesNotMatch(managementRoute,/CROSS JOIN active_tenders tender/);
  assert.match(managementRoute,/current_relevance AS MATERIALIZED\(/);
  assert.match(managementRoute,/current_relevance AS MATERIALIZED\([\s\S]*SELECT candidate\.\*/);
  assert.match(managementRoute,/JOIN tender\.service_relevance_evaluations candidate[\s\S]*candidate\.company_id=scope\.company_id AND candidate\.service_line=scope\.service_line/);
  assert.match(managementRoute,/JOIN active_tenders tender ON tender\.id=candidate\.tender_id/);
  assert.match(managementRoute,/\(\$7::boolean OR candidate\.primary_company\)[\s\S]*NOT EXISTS\(SELECT 1 FROM tender\.service_relevance_evaluations newer/);
  ...
  assert.match(managementRoute,/\(newer\.evaluation_version,newer\.created_at,newer\.id\)>\(candidate\.evaluation_version,candidate\.created_at,candidate\.id\)/);
