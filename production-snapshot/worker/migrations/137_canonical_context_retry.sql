...
SELECT pg_advisory_xact_lock(hashtextextended('wb-tender:canonical-context-retry:137',0));

WITH old_failure AS(
  SELECT DISTINCT ON(queue.tender_id,queue.company_id,queue.lot_key) queue.*
  FROM tender.autopilot_queue queue
  WHERE queue.status='DEAD_LETTER'
    AND coalesce(queue.safe_error_code,queue.error_code)='23502'
    AND queue.action_type='RUN_FULL_PIPELINE'
    AND NOT EXISTS(
      SELECT 1 FROM tender.autopilot_queue newer
      WHERE newer.created_at>queue.created_at
        AND newer.tender_id=queue.tender_id
        AND newer.company_id=queue.company_id
        AND newer.lot_key=queue.lot_key
        AND newer.action_type=queue.action_type
        AND newer.status IN('SUCCEEDED','DONE')
    )
  ORDER BY queue.tender_id,queue.company_id,queue.lot_key,queue.created_at DESC
), authoritative_context AS(
  SELECT failure.*,lot.id canonical_lot_id,
  ...
  FROM old_failure failure
  JOIN tender.tenders tender ON tender.id=failure.tender_id
  ...
  JOIN tender.enterprise_company_links company
    ON company.company_id=failure.company_id AND company.active
  JOIN tender.lots lot
    ON lot.tender_id=failure.tender_id AND lot.external_id=failure.lot_key
  JOIN tender.tender_lot_lifecycles lifecycle
    ON lifecycle.tender_id=failure.tender_id
    AND lifecycle.lot_key=failure.lot_key
  ...
  JOIN LATERAL(
    ...
    WHERE version.tender_id=failure.tender_id
    ORDER BY version.version DESC LIMIT 1
  ) current_tender_version ON true
  JOIN LATERAL(
    SELECT enrichment.id FROM tender.enrichment_versions enrichment
    WHERE enrichment.tender_id=failure.tender_id AND enrichment.historical=false
    ORDER BY enrichment.version DESC LIMIT 1
  ) current_enrichment ON true
  JOIN tender.current_service_relevance relevance
    ON relevance.tender_id=failure.tender_id
    AND relevance.company_id=failure.company_id
    AND relevance.lot_key=failure.lot_key
  ...
  JOIN tender.current_registered_tender_company_portals registered
    ON registered.tender_id=failure.tender_id
    AND registered.company_id=failure.company_id
  ...
  JOIN LATERAL(
    ...
    WHERE region.tender_id=failure.tender_id
      AND region.company_id=failure.company_id
  ) configuration ON configuration.configuration_version_no IS NOT NULL
  JOIN tender.pipeline_contexts pipeline_context
    ON pipeline_context.tender_id=failure.tender_id
    AND pipeline_context.company_id=failure.company_id
  ...
  SELECT gen_random_uuid(),'RUN_FULL_PIPELINE',context.tender_id,
    ...
    encode(digest(concat_ws(':','canonical-context-retry-137',context.tender_id,
  ...
  UNION ALL
  SELECT id,'INSERTED'::text operation FROM inserted
), evidence AS(
...
SELECT 'canonical_context_retry_scheduled',jsonb_build_object(
  'release','20260825-canonical-context-retry-137.1',
  'affectedRows',affected_rows,'insertedRows',inserted_rows,
  'reactivatedRows',reactivated_rows,'rowFingerprint',row_fingerprint,
  'sourceErrorCode','23502','canonicalLotRequired',true,
...
VALUES('0137-canonical-context-retry',
  'Schedule exact canonical replacement jobs for active 23502 failures that used obsolete synthetic lot ids')
