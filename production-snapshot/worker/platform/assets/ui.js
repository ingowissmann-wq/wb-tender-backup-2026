(() => {
  ...
  const tabs = [["overview","Übersicht"],["tenders","Ausschreibungen"],["management-inbox","Management-Inbox"],["deadline-review","Fristenprüfung"],["csm","CSM"],["scheduler","Schedulerstatus"],["favorites","Favoriten"],["deadlines","Fristen"],["tasks","Aufgaben"],["reminders","Wiedervorlagen"],["sources","Quellen"],["imports","Importprotokolle"],["deadletters","Dead Letters"]];
  ...
  async function load(){
    ...
    try{
      ...
      else if(current==="deadline-review"){data=await request("/deadline-review");out.innerHTML=cards(data.items)}
      else if(current==="csm"){location.assign("/saas/app/csm");return}
  ...
  async function detail(id){try{const item=await request("/tenders/"+encodeURIComponent(id));out.innerHTML='<article class="panel"><button type="button" id="back">← Zurück</button><h1>'+esc(item.title)+'</h1><p>'+esc(item.buyer)+'</p><p>'+esc(item.description||"Keine Beschreibung vorhanden.")+'</p><dl><dt>Quelle</dt><dd><a rel="noopener noreferrer" target="_blank" href="'+esc(item.source_url)+'">'+esc(item.source_code)+'</a></dd><dt>Frist</dt><dd>'+esc(item.offer_deadline||"Nicht angegeben")+'</dd><dt>CPV</dt><dd>'+esc((item.cpv_codes||[]).join(", ")||"Nicht angegeben")+'</dd></dl></article>';document.querySelector("#back").onclick=load}catch(error){out.innerHTML='<p class="error" role="alert">'+esc(error.message)+'</p>'}}
