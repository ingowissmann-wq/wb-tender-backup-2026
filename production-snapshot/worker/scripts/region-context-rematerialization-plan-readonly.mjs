...
const apply=process.env.REGION_CONTEXT_REMATERIALIZATION_APPLY==="true";
const expectedPlanHash=String(process.env.REGION_CONTEXT_REMATERIALIZATION_PLAN_SHA256||"").trim();
if(apply&&(
  process.env.EXTERNAL_SUBMISSION_ENABLED!=="false"||
  process.env.WB_TENDER_ALLOW_EXTERNAL_SUBMISSION!=="false"||
  process.env.WB_TENDER_SUBMISSION_GLOBAL_KILL_SWITCH!=="true"
))throw new Error("submission_safety_state_invalid");
const rawPool=new pg.Pool({connectionString,max:3,options:`${apply?"":"-c default_transaction_read_only=on "}-c statement_timeout=60000 -c lock_timeout=3000`});
...
try{
  ...
  const readyHash=digest(ready),plan={schema:"wb-tender/region-context-rematerialization-plan-readonly/1.1.0",capturedAt:new Date().toISOString(),status:"PRODUCTION_READ_ONLY_VERIFIED",transaction:"READ_ONLY_ROLLED_BACK",ready:{count:ready.length,sha256:readyHash,byCompany:summarize(ready),allCanonicalLotsPresent:ready.every(row=>Boolean(row.canonical_lot_id)),allDeadlinesOpen:ready.every(row=>new Date(row.offer_deadline)>new Date())},authoritativeEvidenceRequired:{count:evidenceRequired.length,sha256:digest(evidenceRequired),byCompany:summarize(evidenceRequired)},externalWrite:false,transmitted:false};
  console.log(JSON.stringify(plan,null,2));
  await client.query("ROLLBACK");
  if(apply){
    if(ready.length!==490||readyHash!==expectedPlanHash)throw Object.assign(new Error("authorized_region_plan_mismatch"),{code:"AUTHORIZED_REGION_PLAN_MISMATCH",count:ready.length,sha256:readyHash});
    if(!ready.every(row=>row.company_id==="15c3c602-aa51-4dd4-adc1-3586dc82e523"&&row.canonical_lot_id&&new Date(row.offer_deadline)>new Date()))throw new Error("authorized_region_scope_mismatch");
    const first=ready[0],result=await runInboxPipeline(pool,{contextBindings:ready.map(row=>({tender_id:row.tender_id,company_id:row.company_id,lot_key:row.lot_key})),sourceRunId:null,runKind:"BACKFILL",batchSize:100,scope:{tenantId:first.tenant_id,companyId:first.company_id,canonicalService:first.canonical_service,profileId:first.profile_id}});
    console.log(JSON.stringify({schema:"wb-tender/authorized-region-context-rematerialization/1.0.0",status:result.passed?"APPLIED":"FAILED",authorizedPlanSha256:readyHash,authorizedCount:ready.length,result,externalSubmission:false,externalTransmission:false,transmitted:false},null,2));
  }
