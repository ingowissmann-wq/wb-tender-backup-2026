...
const DOCUMENT_EXTENSIONS =
  /\.(pdf|docx?|xlsx?|ods|csv|xml|json|html?|zip|gaeb|[xdp]8[123])(?:$|[?#])/i;
    ...
    try {
      ...
      const ribDownload = isRibPublicDownload(url),
        ...
        aumassDownload =
          ...
          /^[A-Z0-9._-]{3,80}$/i.test(
            url.searchParams.get("aumassid") || "",
          );
      if (
        DOCUMENT_EXTENSIONS.test(url.href) ||
        ribDownload ||
        evergabeDownload ||
        aumassDownload
      )
...
export async function parseFetched(fetchResult) {
  ...
  if (mime === "application/zip" && fetchResult.buffer.length > 100 * 1024 * 1024) {
    try {
      ...
      return {
        status: "QUARANTINED_PENDING_LEAF_SCAN",
        name,
        mime,
        buffer: fetchResult.buffer,
        malwareScan: { status: "PENDING_LEAF_SCAN", engine: "clamd" },
        parsed: { type: "ZIP", archive, manualReview: false },
      };
    } catch (error) {
      throw Object.assign(
        Error("large archive failed bounded structural validation"),
        { code: "ARCHIVE_STRUCTURAL_VALIDATION_FAILED", cause: error },
      );
    }
  }
...
export async function recoverStoredParserFailures(
  pool,
  enrichmentId = null,
  { parse = parseFetched, verifiedOnly = false, safeRelevantOnly = false } = {},
) {
  const rows = (
    await pool.query(
    ...
    ORDER BY id FOR UPDATE`,
      [enrichmentId, verifiedOnly, safeRelevantOnly],
    )
  ).rows;
  const summary = {
      ...
      await pool.query(
        `SELECT count(*)::int count,
         ...
         FROM tender.enrichment_documents WHERE enrichment_version_id=$1
           AND provenance->>'parentDocumentHash'=$2`,
        [enrichmentId, archive.payload_sha256],
      )
    ).rows[0];
    const leafValidated =
      archive.provenance?.malwareScanStatus === "PENDING_LEAF_SCAN" &&
      Number(materialized?.count || 0) === children.length &&
      materialized?.all_clean === true;
    const archiveEvidence = json({
      archiveExtractionVersion: PIPELINE_VERSION,
      archiveChildrenMaterialized: children.length,
      archiveExtractedAt: new Date().toISOString(),
      ...(leafValidated
        ? {
            malwareScanStatus: "CLEAN_BY_BOUNDED_LEAF_SCAN",
            malwareScanEngine: "clamd",
            leafScanCount: children.length,
          }
        : {}),
    });
    if (leafValidated)
      await pool.query(
        `WITH parent AS(
         ...
         FROM parent WHERE scan.document_id=parent.id AND scan.payload_sha256=parent.payload_sha256
           AND scan.status IN('PENDING','SCAN_ERROR','QUARANTINED')`,
        [archive.id, archiveEvidence],
      );
    else
      await pool.query(
...
async function loadNotice(pool, tender) {
  ...
  const url = `https://ted.europa.eu/en/notice/${encodeURIComponent(tender.external_id)}/xml`,
    fetched = await fetchBounded(url, { maxBytes: 20_000_000 });
  if (fetched.status !== "FETCHED") throw new Error(`notice_${fetched.status}`);
...
async function persistEnrichment(pool, runId, tender, result) {
  result = {
    ...
    documentLinks: Array.isArray(result?.documentLinks)
...
async function authenticatePortal(
  ...
  if (
    ["MFA_BESTÄTIGUNG_ERFORDERLICH", "CAPTCHA_MANUELL_ERFORDERLICH"].includes(
      http.resultCode,
    )
  )
    return http;
...
async function portalReadHeaders(pool, url, companyIds) {
  try {
    ...
    const result = await restoreOrLoginPortalSession(
      ...
      url,
      true,
      { enqueueFanout: false, allowAutomaticLogin: false },
    );
...
async function registeredPublicNetServerPortal(pool, url) {
  ...
  if (
    ...
    !/^54321-(?:Net)?Tender-[a-z0-9-]+$/i.test(
      target.searchParams.get("TenderOID") || "",
    )
  )
    return null;
  return (
...
async function registeredPublicAIBietercockpitPortal(pool, url) {
  ...
  if (
    target.protocol !== "https:" ||
    target.hostname !== "www.deutsches-ausschreibungsblatt.de" ||
    !/^\/VN\/[A-Za-z0-9._-]+$/.test(target.pathname)
  )
    return null;
  return (
    (
      await pool.query(
        `SELECT id,canonical_domain,adapter_id FROM tender.portal_registry
         WHERE canonical_domain=$1 AND adapter_id='ai-vergabe-manager' LIMIT 1`,
        [target.hostname],
      )
    ).rows[0] || null
  );
}
async function portalContext(pool, initialUrl, finalUrl, companyIds) {
  ...
  if (scopedCompanies.length !== 1)
    return {
      status: "PORTALZUGANG_NICHT_KONFIGURIERT",
      domain: final,
      portal: null,
    };
  const companyId = scopedCompanies[0];
  ...
  if (row.session_status)
    return { status: "PORTALSESSION_ABGELAUFEN", domain: final, portal: row };
  return { status: "LOGIN_ERFORDERLICH", domain: final, portal: row };
}
async function processDeutscheEvergabe(pool, enrichment, selected, document) {
  ...
  if (companyIds.length !== 1) {
    await pool.query(
      "UPDATE tender.enrichment_documents SET fetch_status='PORTALZUGANG_NICHT_KONFIGURIERT',resolution_status='PORTAL_ACCESS_REQUIRED',parser='PORTAL_REDIRECT',parser_version=$2,retrieved_at=now(),provenance=provenance||$3::jsonb WHERE id=$1",
      [
        document.id,
        PIPELINE_VERSION,
        json({
  ...
  const credential = (
    await pool.query(
      `SELECT c.* FROM tender.portal_credential_secrets c JOIN tender.portal_credential_companies cc ON cc.credential_id=c.id WHERE c.portal_id=$1 AND c.status='ACTIVE' AND cc.company_id=$2::uuid AND cc.active=true ORDER BY c.version DESC LIMIT 1`,
      [portal.id, companyId],
    )
  ).rows[0];
  if (!credential) {
...
async function processDocuments(pool, enrichment, selected = []) {
  ...
  const documents = (
      await pool.query(
        "SELECT * FROM tender.enrichment_documents WHERE enrichment_version_id=$1 AND NOT(document_type IN ('PORTAL_TENDER_DOCUMENT','INTERNAL_ACCEPTANCE_DOCUMENT') AND resolution_status='DOWNLOAD_SUCCEEDED' AND content IS NOT NULL) AND NOT(parser='HTML' AND fetch_status='VORHANDEN' AND coalesce((provenance->>'discoveredFiles')::int,0)>0 AND retrieved_at>now()-interval '10 minutes') ORDER BY source_url",
        [enrichment.id],
      )
    ).rows,
    seen = new Set();
  ...
  for (const document of documents) {
    ...
    const source = new URL(accessUrl),
      ...
      publicNetServerPortal = await registeredPublicNetServerPortal(
        pool,
        accessUrl,
      ),
    ...
    let fetched = publicEvergabeOnline
      ...
      : publicNetServerPortal
        ...
        : publicAIBietercockpitPortal
          ? await downloadPublicAIBietercockpitArchive(accessUrl).catch(
              (error) => ({
                status: "DOWNLOAD_FEHLGESCHLAGEN",
                error: String(error.message).slice(0, 120),
              }),
    ...
    await pool.query(
      "UPDATE tender.enrichment_documents SET lot_id=coalesce(lot_id,$2),filename=$3,fetch_status=$4,http_status=$5,mime_type=$6,payload_sha256=$7,content=$8,extracted_data=$9::jsonb,parser=$10,parser_version=$11,retrieved_at=now(),provenance=provenance||$12::jsonb WHERE id=$1",
      [
        document.id,
        binding.lotId,
        parsed.name,
...
async function materializeDocumentFacts(
  ...
  for (const fact of deriveCleaningRoomBookFacts(
    ...
    await pool.query(
      `INSERT INTO tender.enrichment_fields(enrichment_version_id,field_key,value,quality_status,provenance,confidence) SELECT $1,$2,$3::jsonb,'VORHANDEN',$4::jsonb,0.99 WHERE NOT EXISTS(SELECT 1 FROM tender.enrichment_fields WHERE enrichment_version_id=$1 AND field_key=$2 AND provenance->>'parser'='cleaning-room-book-v4' AND provenance->>'selectedLotId' IS NOT DISTINCT FROM $5)`,
      [
        enrichmentId,
        fact.key,
        json(fact.value),
...
async function relevancePlan(pool, tender, enrichment) {
  ...
  const explicitInputRows = (
    ...
    derivedContractFacts =
      item.service_scope === "cleaning"
        ? deriveCleaningContractFacts(documents, item.lot_key)
        : [],
    derivedDuration =
  ...
  const engineResult = calculateSectorTender({
    ...
    facts: {
      ...
      duration:
        ...
        valueFor(result.review.procurement, "Vertragslaufzeit"),
      areas: valueFor(result.review.scope, "Flächen"),
      nightHours: valueFor(result.review.scope, "Nachtstunden"),
      sundayHours: valueFor(result.review.scope, "Sonntagsstunden"),
      holidayHours: valueFor(result.review.scope, "Feiertagsstunden"),
      ...
      pricePositions: valueFor(result.review.scope, "Preispositionen"),
    },
    provenance: {
      ...calculationInput.provenance,
      contractDuration: derivedDuration
...
async function persistManagementOutput(
  ...
  try {
    await client.query("BEGIN");
    // A partial unique index cannot serialize the first insert when no
    // current row exists yet. Lock the stable tender identity so concurrent
    // pipeline jobs for the same tender cannot both observe an empty current
    // management-output slot.
    await client.query(
...
async function ensureManagementApprovalRequest(
  ...
  try {
    ...
    const existing = (
      await client.query(
        "SELECT id,status FROM tender.approval_requests WHERE tender_id=$1 AND action_type='BID_SUBMISSION' AND payload_sha256=$2 LIMIT 1",
        [tender.id, binding.sha256],
      )
    ).rows[0];
    if (existing) {
    ...
    await client.query(
      "UPDATE tender.approval_requests SET status='SUPERSEDED' WHERE tender_id=$1 AND action_type='BID_SUBMISSION' AND payload_manifest->>'lotKey'=$2 AND payload_manifest->>'companyId'=$3 AND status IN ('DRAFT','REQUESTED','APPROVED')",
      [tender.id, item.lot_key || "GLOBAL", String(item.company_id)],
    );
    const expiresAt =
    ...
    const row = (
      await client.query(
        `INSERT INTO tender.approval_requests(tender_id,action_type,payload_sha256,payload_manifest,tender_version_id,calculation_id,status,requested_by,expires_at) VALUES($1,'BID_SUBMISSION',$2,$3::jsonb,$4,$5,'REQUESTED',$6,$7) RETURNING id,status`,
        [
          tender.id,
          binding.sha256,
          json(payload),
...
async function processAction(pool, item, tender, run) {
  ...
  if (
    ...
    const ds = (
      await pool.query(
        `SELECT count(*) FILTER(WHERE document_type='PORTAL_TENDER_DOCUMENT' AND coalesce(procurement_relevant,true) AND coalesce(document_class,'')<>'GENERAL_PORTAL_DOCUMENT')::int found,count(*) FILTER(WHERE procurement_verification_status='VERIFIED' AND coalesce(procurement_relevant,true) AND coalesce(document_class,'')<>'GENERAL_PORTAL_DOCUMENT')::int downloaded,count(*) FILTER(WHERE procurement_verification_status='VERIFIED' AND parser IS NOT NULL AND coalesce(procurement_relevant,true) AND coalesce(document_class,'')<>'GENERAL_PORTAL_DOCUMENT')::int analyzed,max(coalesce(provenance->>'targetPortal',provenance->>'portal')) portal,(array_agg(coalesce(resolution_status,fetch_status) ORDER BY CASE coalesce(resolution_status,fetch_status) WHEN 'DOWNLOAD_FAILED' THEN 1 WHEN 'PORTAL_ACCESS_REQUIRED' THEN 2 WHEN 'EXTERNAL_DOCUMENT_REQUEST_REQUIRED' THEN 3 ELSE 9 END) FILTER(WHERE coalesce(document_class,'')<>'GENERAL_PORTAL_DOCUMENT' AND document_type IN('PORTAL_TENDER_DOCUMENT','TENDER_DOCUMENT')))[1] access FROM tender.enrichment_documents d WHERE enrichment_version_id=$1 AND (d.lot_id IS NULL OR EXISTS(SELECT 1 FROM tender.enrichment_lots l WHERE l.id=d.lot_id AND l.lot_key=$2))`,
        [enrichment.id, item.lot_key],
      )
    ).rows[0];
    const resolution =
  ...
  if (action === "RUN_FULL_PIPELINE") {
    ...
    await pool.query(
      ...
      WHEN portal_access_status='PORTALSESSION_ABGELAUFEN' THEN 'READ_ONLY_SESSION_ERNEUERN'
      WHEN portal_access_status='LOGIN_ERFORDERLICH' THEN 'PORTAL_LOGIN_PRÜFEN'
...
export async function processQueueItem(pool, item) {
  ...
  tender.offer_deadline = lot.offer_deadline;
  const run = (
    await pool.query(
  ...
  try {
    const registered = (
      await pool.query(
        "SELECT 1 FROM tender.current_tender_company_portal_credential_scopes WHERE tender_id=$1::uuid AND company_id=$2::uuid AND portal_id=$3::uuid AND credential_id=$4::uuid LIMIT 2",
        [item.tender_id, item.company_id, item.portal_id, item.credential_id],
      )
    ).rows;
    ...
    await pool.query(
      "UPDATE tender.autopilot_queue SET status='SUCCEEDED',current_step='COMPLETED',progress_percent=100,finished_at=now(),heartbeat_at=now(),result_summary=$2::jsonb,error_code=NULL,safe_error_code=NULL,error_detail_safe=NULL WHERE id=$1",
    ...
    if (code === "NOT_ELIGIBLE") {
      ...
      await pool.query(
        "UPDATE tender.autopilot_queue SET status='SUCCEEDED',current_step='SUPERSEDED_BY_CURRENT_RELEVANCE',progress_percent=100,finished_at=now(),heartbeat_at=now(),result_summary=$2::jsonb,error_code=NULL,safe_error_code=NULL,error_detail_safe=NULL WHERE id=$1",
        [item.id, json(summary)],
      );
      await pool.query(
...
export { materializeDocumentContract };
