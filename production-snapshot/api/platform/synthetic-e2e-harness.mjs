...
const baseParameters={C01:20,C03:5,C04:22,C05:173.33,C06:10,C07:5,C08:8,C09:100,C10:120,C11:500,C12:300,C13:100,C14:400,C15:200,C16:50,C17:50,C18:3,C19:5,C20:7,C21:10,S01:200,S02:5,S03:3,S04:150};
  ...
  try{
    ...
    for(const companyType of SYNTHETIC_E2E_COMPANY_TYPES){
      ...
      for(const family of SYNTHETIC_E2E_PORTAL_FAMILIES){
        ...
        const blocked=calculateSectorTender({serviceArea:company.serviceArea,parameters:baseParameters,facts:{duration:12}});assert.equal(blocked.status,"CALCULATION_BLOCKED_MISSING_INPUT");const calculation=calculateSectorTender({serviceArea:company.serviceArea,parameters:baseParameters,facts:{productiveHours:1200,workdays:250,duration:12,areas:500},provenance:{TEST_ONLY:true,SYNTHETIC:true,testRunId,profileRevision:profile.revision}});assert.equal(calculation.status,"CALCULATED");const calculationRow={id:id(testRunId,"calculation",companyType.key,family),companyId,tenderId,lotId:tender.lotId,version:2,missingInputsInitially:blockerNames(blocked),calculation,...tag(testRunId,createdAt,expiresAt)};state.calculations.put(calculationRow);step("CALCULATE_WITH_MISSING_INPUT_GATE");step("SUPPLY_MISSING_VALUES_AND_RECALCULATE");
