...
The checkout must be clean and exactly match `EXPECTED_COMMIT` and `EXPECTED_TREE`. Install its production dependencies from the reviewed lockfile with `npm ci --omit=dev --audit=false --fund=false`; an import and `npm ls` preflight proves the host-side IAM and browser gate runtime before any backup is generated. `RELEASE_IMAGE`, `POSTGRES_IMAGE`, and `PRODUCTION_BROWSER_IMAGE` must be registry-digest references. The release image ID must equal `EXPECTED_RELEASE_IMAGE_ID`, and its OCI `org.opencontainers.image.revision` and `org.opencontainers.image.source-tree` labels must equal the approved commit and tree. Isolated restore tools execute the source and production dependencies embedded in that exact release image; the host checkout must never be mounted over `/app`, because that would mask the image's `node_modules` and make the gate depend on undeclared host state. The browser image must be the reviewed Playwright runtime used by rehearsal and must already be present locally by its exact digest.
...
Before touching the live schema, the rollout creates a new encrypted custom-format logical dump with the existing file-based key. It verifies the ciphertext checksum, decrypts and verifies the plaintext checksum, and reads the `pg_restore` catalog. That exact new ciphertext is then restored into a temporary PostgreSQL container attached only to a newly created Docker `--internal` network. Because database-only logical dumps do not include cluster roles, the verifier idempotently creates the non-login, non-privileged `tender_api_runtime` role before role-targeted migrations and rejects any unsafe pre-existing definition. Only missing release migrations are applied there, their exact least-privilege login-challenge grants are verified, followed by the real tenant and admin database isolation programs and exact server-side Pro 99000, Business 149000, and Enterprise 249000 minor-unit boundaries. The container, volume, temporary credentials, and internal network are removed on every exit; the encrypted backup and checksum-bound manifest remain outside the checkout.

Immediately before live migration, exact API, worker, and scheduler image IDs, commands, environments, health states, and restart counters are recorded along with schema, commercial-plan, migration-ledger, and migration-snapshot hashes. Whole-schema snapshots run only through the existing Compose database client and its file-based restore-admin `PGPASSFILE`; the least-privilege API login is never broadened to support rollout administration. Only migrations absent from the checksum ledger run. On failure, API, Worker, and Scheduler are stopped; sessions belonging exactly to `wb_tender_api_login` in the current database are terminated and verified absent; and only migrations attempted by this release are taken down in reverse order. The three exact prior image, command, and environment sets are restored only after that reverse-migration attempt, preventing runtime transactions from blocking concurrent index rollback and preventing candidate runtime configuration from being combined with a prior image. Candidate-only environment keys are explicitly reset rather than inherited through the Compose merge. The complete database state hashes and service image, command, environment, and health states are compared afterward. Any rollback or verification error exits 90 with `EMERGENCY_ROLLBACK_VERIFICATION_FAILED`; external submission stays disabled. The rollout never restores the backup over the live database automatically.
