...
Das bestehende Produktionsmonitoring bleibt standardmäßig auf dem bisherigen deaktivierten Abgabebetrieb. Nach bestandener Abnahme wird `WB_TENDER_MONITOR_SUBMISSION_ENABLED=true` für den Monitor gesetzt. Dann sind auch `wb-tender-production-submission-worker`, identische Images und `DEDICATED_VALIDATED_WORKER` für alle vier Anwendungsdienste erforderlich. Die interne Worker-Healthantwort muss zu dessen Rolle und zum Commit des API-Images passen und vollständige, nicht negative Metriken liefern. Fehlende Heartbeats oder technische Workerfehler lösen den vorhandenen Betriebsalarm aus. Wartende Portalzugänge sind Geschäftsstatus und werden über die bereits vorhandenen Jobbenachrichtigungen gemeldet; ihre Anzahl allein gilt nicht als technische Produktionsregression.
