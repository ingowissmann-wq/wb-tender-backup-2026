...
export const hasPdfAcroForm = (content) => {
  if (!Buffer.isBuffer(content) || content.length < 8) return false;
  const structural = content.toString("latin1");
  return structural.startsWith("%PDF-") && /\/AcroForm\b/.test(structural) && /\/Fields\s*\[/.test(structural);
};

export function resolveRequiredOriginalForm(requirement = {}, candidates = []) {
  ...
  const editable = EDITABLE.has(mime) || (mime === PDF && hasPdfAcroForm(candidate.content));
  return {
