  ...
  try{
    ...
    for(const companyType of SYNTHETIC_E2E_COMPANY_TYPES){
      ...
      for(const family of SYNTHETIC_E2E_PORTAL_FAMILIES){
        ...
        const blocked=calculateSectorTender({serviceArea:company.serviceArea,parameters:baseParameters,units:baseUnits,facts:{duration:12}});assert.equal(blocked.status,"CALCULATION_BLOCKED_MISSING_INPUT");const calculation=calculateSectorTender({serviceArea:company.serviceArea,parameters:baseParameters,units:baseUnits,facts:{productiveHours:1200,workdays:250,duration:12,areas:500,unitCount:1},provenance:{TEST_ONLY:true,SYNTHETIC:true,testRunId,profileRevision:profile.revision}});assert.equal(calculation.status,"CALCULATED");const calculationRow={id:id(testRunId,"calculation",companyType.key,family),companyId,tenderId,lotId:tender.lotId,version:2,missingInputsInitially:blockerNames(blocked),calculation,...tag(testRunId,createdAt,expiresAt)};state.calculations.put(calculationRow);step("CALCULATE_WITH_MISSING_INPUT_GATE");step("SUPPLY_MISSING_VALUES_AND_RECALCULATE");
