...
async function targetRows(client,tenderIds,scope={},contextBindings=[]){
  ...
  return (await client.query(`SELECT DISTINCT ON(t.id,r.company_id,r.lot_key) t.*,r.company_id,r.lot_key,canonical_lot.id canonical_lot_id,r.evaluation_version relevance_version,r.snapshot_sha256 relevance_snapshot,r.service_line,r.relevance_status,r.service_scope_gate,r.reason relevance_reason,c.legal_name,c.technical_key,c.sector_slug,c.sector_status,tv.id tender_version_id,tv.normalized_data,configuration_scope.tenant_id,configuration_scope.canonical_service,configuration_scope.profile_id
    ...
    LEFT JOIN tender.lots canonical_lot ON canonical_lot.tender_id=t.id AND canonical_lot.external_id=r.lot_key
    WHERE t.id=ANY($1::uuid[]) AND t.data_class='PUBLIC_REAL' AND t.source_lifecycle_status='ACTIVE' AND t.participation_status IN('ELIGIBLE','PARTIALLY_ELIGIBLE')
      ...
      AND ($6::jsonb IS NULL OR EXISTS(SELECT 1 FROM jsonb_to_recordset($6::jsonb) binding(tender_id uuid,company_id uuid,lot_key text)
        WHERE binding.tender_id=t.id AND binding.company_id=r.company_id AND binding.lot_key IS NOT DISTINCT FROM r.lot_key))
    ...
    ORDER BY t.id,r.company_id,r.lot_key,r.evaluation_version DESC`,[tenderIds,scope.companyId||null,scope.canonicalService||"",scope.tenantId||null,scope.profileId||null,contextBindings.length?json(contextBindings):null])).rows;
}
