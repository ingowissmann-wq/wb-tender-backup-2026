...
import pg from "pg";
import { configuredOwnerEmail, passwordHash } from "./owner-auth.mjs";
...
try {
  ...
  if (users.rowCount && !(users.rowCount === 1 && users.rows[0].email.toLowerCase() === ownerEmail))
  ...
  if (!user) user = (await client.query("INSERT INTO iam.users(email,password_hash,active,mfa_required) VALUES($1,$2,true,true) RETURNING id,email", [ownerEmail, await passwordHash(discardedPassword)])).rows[0];
