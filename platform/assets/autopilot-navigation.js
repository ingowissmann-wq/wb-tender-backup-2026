...
<details class="panel calc-technical"><summary>Revisionsnachweise</summary>${calcFacts(
  ...
  async function portalForm(portal, companyId, { locked = false } = {}) {
    ...
    if (metadataForm) metadataForm.onsubmit = async (event) => {
      ...
      try {
        ...
        const firstField=Object.keys(error.fieldErrors||{})[0],field=firstField?metadataForm.elements.namedItem(firstField):null;if(field&&typeof field.focus==="function")field.focus();
        status.textContent = `Zugangsmetadaten konnten nicht gespeichert werden: ${error.message} (Fehler-ID: ${error.requestId})`;
      }
    };
  ...
  async function portalView() {
    ...
    if(d.items.some(p=>p.canManage)){
      const recovery=document.createElement("section");recovery.className="panel";
      recovery.innerHTML='<h2>Portalzugänge sicher erneut eingeben</h2><p data-recovery-status role="status"></p><div data-recovery-items></div>';
      out.append(recovery);
      import("/submission-credential-recovery.js").then(module=>module.mountCredentialRecovery(recovery,{companyId:d.selectedCompany?.id||null})).catch(()=>{recovery.querySelector("[data-recovery-status]").textContent="Die sichere Neueingabe konnte nicht geladen werden. Bitte die Portalverwaltung erneut öffnen.";});
    }
    let searchTimer;
  ...
  async function globalView(s) {
    ...
    if(s.view==="regions"){
      ...
      out.innerHTML=title("Regionale Zonen","Jede fachliche Änderung erzeugt eine unveränderliche neue Version. Löschen ist technisch gesperrt.")+`<section class="panel"><p><strong>Versionsverwaltung aktiv.</strong> Bearbeiten, Umzug und Stilllegung erhalten die vollständige Historie und Auditspur.</p>${d.canManage?'<button type="button" id="region-create">Neue Zone anlegen</button>':""}</section><section class="region-zone-grid">${d.groups.map(g=>{const x=g.current;return `<article class="panel region-zone-card" data-region-zone="${esc(x.id)}"><header><h2>${esc(x.name)}</h2><span class="status-badge">${esc(status[x.lifecycle_status]||x.lifecycle_status)}</span></header><dl><dt>Code / Version</dt><dd><strong>${esc(x.code)}</strong> / ${esc(x.version)}</dd><dt>Gebiete</dt><dd>${esc((x.regions?.areas||[]).join(", "))}</dd><dt>Gültig ab</dt><dd>${esc(calcDate(x.effective_from))}</dd><dt>Änderungsart</dt><dd>${esc(change[x.change_type]||x.change_type)}</dd><dt>Begründung</dt><dd>${esc(x.change_reason)}</dd><dt>Fingerprint</dt><dd><code>${esc(x.content_sha256)}</code></dd></dl>${d.canManage&&x.lifecycle_status==="ACTIVE"?`<div class="review-actions"><button type="button" data-region-version="${esc(x.id)}">Neue Version</button><button type="button" data-region-relocate="${esc(x.id)}">Umzugsworkflow</button><button type="button" data-region-retire="${esc(x.id)}">Stilllegen</button></div>`:""}<details><summary>Versionshistorie (${esc(g.history.length)})</summary><div class="table-wrap"><table><thead><tr><th>Version</th><th>Status</th><th>Vorgang</th><th>Wirksam</th><th>Begründung</th><th>SHA-256</th></tr></thead><tbody>${history([x,...g.history])}</tbody></table></div></details></article>`}).join("")}</section><dialog id="region-zone-dialog"></dialog>`;
      const byId=Object.fromEntries(d.items.map(x=>[x.id,x])),dialog=out.querySelector("#region-zone-dialog"),today=new Date().toISOString().slice(0,10);
      const openForm=(zone=null,mode="CREATE")=>{const relocation=mode==="RELOCATION",regions=zone?.regions||{areas:[]},travel=zone?.travel_rules||{};dialog.innerHTML=`<form method="dialog" class="panel region-zone-form"><h2>${mode==="CREATE"?"Neue regionale Zone":relocation?"Umzug als neue Version":"Neue Zonenversion"}</h2><label>Zonencode<input name="code" required maxlength="40" value="${esc(zone?.code||"")}" ${zone?"readonly":""}></label><label>Name<input name="name" required maxlength="160" value="${esc(zone?.name||"")}"></label><label>Gebiete – ein Eintrag je Zeile<textarea name="areas" required>${esc((regions.areas||[]).join("\n"))}</textarea></label><label>Fahrt- und Bewertungsregeln (JSON)<textarea name="travelRules" required>${esc(JSON.stringify(travel,null,2))}</textarea></label><label>Wirksam ab<input name="effectiveFrom" type="date" required value="${esc(today)}"></label>${relocation?'<label>Ausgangsort<input name="relocationFrom" required></label><label>Zielort<input name="relocationTo" required></label>':""}<label>Änderungsbegründung<textarea name="reason" required maxlength="2000"></textarea></label><p role="alert" class="error" data-region-error hidden></p><div class="review-actions"><button type="submit" value="save">${mode==="CREATE"?"Zone anlegen":"Neue Version speichern"}</button><button type="submit" value="cancel">Abbrechen</button></div></form>`;dialog.showModal();const form=dialog.querySelector("form");form.addEventListener("submit",async event=>{event.preventDefault();if(event.submitter?.value==="cancel"){dialog.close();return}const f=new FormData(form),error=form.querySelector("[data-region-error]");let travelRules;try{travelRules=JSON.parse(String(f.get("travelRules")||"{}"))}catch{error.hidden=false;error.textContent="Fahrt- und Bewertungsregeln enthalten kein gültiges JSON.";return}const body={code:f.get("code"),name:f.get("name"),regions:{...regions,areas:String(f.get("areas")||"").split(/\r?\n/).map(v=>v.trim()).filter(Boolean)},travelRules,effectiveFrom:f.get("effectiveFrom"),reason:f.get("reason"),changeType:mode,...(relocation?{relocation:{from:f.get("relocationFrom"),to:f.get("relocationTo"),effectiveAt:f.get("effectiveFrom")}}:{})};if(zone)body.expectedContentSha256=zone.content_sha256;try{if(zone)await mutate(`/regions/${zone.id}/versions`,"POST",body);else await mutate("/regions","POST",body);dialog.close();await render()}catch(e){error.hidden=false;error.textContent=`${e.message} (Fehler-ID: ${e.requestId||"nicht verfügbar"})`}})};
      out.querySelector("#region-create")?.addEventListener("click",()=>openForm());
      for(const button of out.querySelectorAll("[data-region-version]"))button.addEventListener("click",()=>openForm(byId[button.dataset.regionVersion],"UPDATE"));
      for(const button of out.querySelectorAll("[data-region-relocate]"))button.addEventListener("click",()=>openForm(byId[button.dataset.regionRelocate],"RELOCATION"));
      for(const button of out.querySelectorAll("[data-region-retire]"))button.addEventListener("click",async()=>{const zone=byId[button.dataset.regionRetire];if(!confirm(`${zone.name} als neue, auditierbare Version stilllegen?`))return;const reason=prompt("Begründung für die Stilllegung (Pflichtfeld)")||"",effectiveFrom=prompt("Wirksamkeitsdatum (JJJJ-MM-TT)",today)||"";if(!reason)return;try{await mutate(`/regions/${zone.id}/retire`,"POST",{expectedContentSha256:zone.content_sha256,reason,effectiveFrom});await render()}catch(e){alert(`${e.message} (Fehler-ID: ${e.requestId||"nicht verfügbar"})`)}});
      return;
    }
    const map = {
        sources: "/sources",
  ...
  async function contextView(s) {
    ...
    if (s.view === "detail") {
      ...
      if (portals.items.length) {
        ...
        out.insertAdjacentHTML(
          "beforeend",
          `<section class="panel"><h3>Detailunterlagen und Portalzugang</h3><p>${esc(portals.reason || "Vergabeportal in der Quelle nicht angegeben")}</p><a class="button-link" href="${esc(portalSearchUrl)}">Portalzugang verwalten</a></section>`,
        );
      }
  ...
  async function submissionStatusView(s) {
    ...
    const [feedback,releases,liveSubmission] = await Promise.all([
      ...
      get(`/submission/v2/by-context?tenderId=${encodeURIComponent(s.tender)}&companyId=${encodeURIComponent(s.company)}&lotKey=${encodeURIComponent(s.lot || "")}`).catch(error=>error.status===403?{item:null,forbidden:true}:{item:null}),
    ...
    out.innerHTML =
      ...
      `<section class="panel submission-status-grid">${steps.map(([label, status]) => `<article><strong>${esc(label)}</strong><span>${esc(status)}</span></article>`).join("")}</section><section class="panel submission-blockers-panel"><p class="notice"><strong>Interne Vorbereitung:</strong> Paket, Kalkulation, Portalbindung und Nachweise werden weiterhin ohne Portalübertragung vorbereitet.</p><h3>Konkrete Blocker</h3>${blockers.length ? `<ul class="submission-blocker-list">${blockers.map((blocker) => submissionBlockerItem(blocker, s)).join("")}</ul>` : "<p>Keine internen Blocker festgestellt.</p>"}<h3>Nächste Aktion</h3><div class="review-actions"><button type="button" id="run-submission-preflight">Preflight erneut prüfen</button><button type="button" id="freeze-package-manifest">Paketstand unveränderlich sichern</button><button type="button" id="request-binding-release" ${(feedback.manifests||[]).length&&context.credential_id?"":"disabled aria-disabled=\"true\""}>Interne finale Freigabe anfordern</button><label class="reconciliation-kind">Lesender Rückkanal<select id="reconciliation-job-kind"><option value="READ_ONLY_STATUS_POLL">Status</option><option value="RECEIPT_RECONCILIATION">Empfangsbeleg</option><option value="MESSAGE_POLL">Nachrichten</option><option value="AMENDMENT_POLL">Nachträge</option><option value="DEADLINE_POLL">Fristen</option><option value="OUTCOME_POLL">Zuschlag / Absage / Aufhebung</option></select></label><button type="button" id="poll-portal-status">Read-only Prüfung vormerken</button></div><p id="submission-action-status" aria-live="polite">Interne Prüfungen bereit.</p></section><section class="panel binding-release-panel"><h3>Exakt gebundene Vier-Augen-Freigaben</h3><p>Jede Freigabe gilt nur für Gesellschaft, Portalzugang, Tender, Los und den angezeigten Paket-Hash und läuft automatisch ab.</p>${releaseItems||"<p>Noch keine interne finale Freigabe angefordert.</p>"}</section><section class="panel live-submission-panel"><h3>Rechtsverbindliche externe Angebotsabgabe</h3><p><a href="/admin/ausschreibungen/portalzugang-neueingabe">Nicht entschlüsselbare Portalzugänge sicher erneut eingeben</a></p><p class="notice"><strong>${esc(liveSubmission.dispatch?.status||liveSubmission.item?.status||"NOT_READY")}</strong> · Nur ein produktiv validierter und portalbezogen freigegebener Adapter kann senden.</p><p>${esc(liveSubmission.legalConfirmationText||"Mit dieser Freigabe wird das Angebot nach Abschluss aller technischen Prüfungen rechtsverbindlich an das ausgewählte Vergabeportal übermittelt.")}</p><div class="review-actions"><button type="button" id="prepare-live-submission" ${liveSubmission.item||liveSubmission.forbidden||!context.portal_id||!context.credential_id||!context.bid_package_id?'disabled aria-disabled="true"':""}>Neue versionsgebundene Sendefreigabe vorbereiten</button><button type="button" id="approve-live-submission" ${liveSubmission.item?.status==="APPROVAL_PENDING"?"":'disabled aria-disabled="true"'}>Als Verantwortlicher verbindlich zur automatischen Abgabe freigeben</button></div>${liveSubmission.dispatch?.receipt_sha256?`<p><a href="${esc(API+"/submission/v2/"+liveSubmission.item.id+"/dispatch-receipt")}">Portalbeleg herunterladen</a> · ${esc(liveSubmission.dispatch.portal_reference)}</p>`:""}<p id="live-submission-status" aria-live="polite">${liveSubmission.forbidden?"Keine Submission-Rolle zugeordnet.":"Alle serverseitigen Gates werden unmittelbar vor jeder Phase erneut geprüft."}</p></section><section class="panel portal-feedback"><h3>Portal-Rückkanal und Historie</h3>${feedbackTimeline?`<ol class="portal-feedback-list">${feedbackTimeline}</ol>`:"<p>Noch keine verifizierten eingehenden Portalereignisse vorhanden.</p>"}</section><details class="technical-details"><summary>Technische Details und Audit</summary><dl><dt>Interne Vorbereitungs-ID</dt><dd><code>${esc(context.id)}</code></dd><dt>Audit</dt><dd><code>${esc(context.audit_id)}</code></dd><dt>Binding</dt><dd><code>${esc(context.binding_sha256)}</code></dd>${liveSubmission.item?`<dt>Submission-ID</dt><dd><code>${esc(liveSubmission.item.id)}</code></dd><dt>Idempotency-Key</dt><dd><code>${esc(liveSubmission.item.idempotency_key)}</code></dd>`:""}</dl></details>`;
    ...
    const liveStatus=document.querySelector("#live-submission-status"),legalText=liveSubmission.legalConfirmationText||"Mit dieser Freigabe wird das Angebot nach Abschluss aller technischen Prüfungen rechtsverbindlich an das ausgewählte Vergabeportal übermittelt.";
    document.querySelector("#prepare-live-submission")?.addEventListener("click",async()=>{try{await mutate("/submission/v2/prepare","POST",{tenderId:s.tender,companyId:s.company,lotKey:s.lot||"",bidPackageId:context.bid_package_id,portalId:context.portal_id,credentialId:context.credential_id,idempotencyKey:crypto.randomUUID()});liveStatus.textContent="Neue Freigabe für exakt diesen Paket-, Credential-, Los- und Friststand angelegt.";await submissionStatusView(s)}catch(error){liveStatus.textContent=error.blockers?.length?`Nicht bereit: ${error.blockers.map(item=>item.code).join(", ")}`:error.message}});
    document.querySelector("#approve-live-submission")?.addEventListener("click",async()=>{try{const preview=await get(`/submission/v2/${encodeURIComponent(liveSubmission.item.id)}/dispatch-preview`);const b=preview.binding;const accepted=confirm(`${preview.confirmation}\n\nGesellschaft: ${b.companyId}\nLos: ${b.lotKey}\nPortal: ${b.portalHost}\nFrist: ${b.deadlineAt}\nAngebots-Hash: ${b.packageSha256}`);if(!accepted)return;const result=await mutate(`/submission/v2/${encodeURIComponent(liveSubmission.item.id)}/approve`,"POST",{confirmation:preview.confirmation,bindingSha256:preview.bindingSha256});liveStatus.textContent=`Dauerhafter Abgabejob ${result.id}: ${result.status}`;await submissionStatusView(s)}catch(error){liveStatus.textContent=error.message}});
  }
  async function load() {
    const s = state();
    navigation();
...
(() => {
  ...
  const api = document.body.dataset.api || "/api/tender",
    ...
    post = async (path, body = {}) => {
      ...
      if (!response.ok)
        throw Object.assign(Error(data.message || (response.status === 401 ? "Anmeldung oder MFA-Sitzung erforderlich." : response.status === 403 ? "Keine Berechtigung für diese Aktion." : response.status === 409 ? "Der Stand hat sich geändert. Bitte laden Sie die Ansicht neu." : response.status === 423 ? "Diese rechtlich bindende Portalaktion ist gesperrt. Es wurde nichts übermittelt." : response.status >= 500 ? "Die Aktion konnte wegen eines technischen Fehlers nicht abgeschlossen werden." : `Die Aktion konnte nicht abgeschlossen werden (${response.status}).`)), {
          requestId: data.errorId || data.requestId || response.headers.get("x-request-id") || null,
          status: response.status,
        });
      return data;
    },
    correlatedPortalLogins = new Map(),
