(() => {
  ...
  const recordOrNull = (value) => value && typeof value === "object" && !Array.isArray(value) ? value : null;
  const portalDocumentList = (documents) => {
    const items = (Array.isArray(documents) ? documents : []).filter(recordOrNull).map((document) => {
  ...
  async function mutate(path, method = "POST", body = {}) {
    ...
    if (!r.ok) {const error=Error(data.message || (r.status === 401 ? "Anmeldung oder MFA-Sitzung erforderlich." : r.status === 403 ? "Keine Berechtigung für diese Aktion." : r.status === 409 ? "Der Stand hat sich geändert. Bitte laden Sie die Ansicht neu." : r.status === 423 ? "Diese rechtlich bindende Portalaktion ist gesperrt. Es wurde nichts übermittelt." : r.status >= 500 ? "Die Aktion konnte wegen eines technischen Fehlers nicht abgeschlossen werden." : `Die Aktion konnte nicht abgeschlossen werden (${r.status}).`));error.requestId=data.requestId||r.headers.get("x-request-id")||"nicht verfügbar";error.status=r.status;error.code=data.error||null;error.currentCredentialVersion=data.currentCredentialVersion??null;throw error}
    if (data.job_id) {
      const portalId =
  ...
  const correlatedPortalLogins = new Map(),
    reconnectFailureStatuses = new Set(["LOGIN_FAILED","SESSION_EXPIRED","MFA_REQUIRED","WRONG_ACCOUNT_CONTEXT","WRONG_ORGANIZATION_CONTEXT","LOGIN_FORM_CHANGED"]),
    reconnectErrorText = (state, error) => {
      const id = state?.errorId || error?.requestId || "nicht verfügbar";
      return `${state?.message || error?.message || "Die Portalverbindung konnte nicht bestätigt werden."}${state?.detail ? ` Technische Diagnose: ${state.detail}.` : ""} Fehler-ID: ${id}. Sie können den Vorgang sicher erneut starten.`;
    };
  ...
  async function calculationView(s) {
    ...
    try {
      d = await get(calculationPath);
    } catch (error) {
      if (
        error.status !== 404 ||
        (error.code && error.code !== "calculation_not_found")
      )
        throw error;
      ...
      out.innerHTML =
        title(
          ...
          "Für diesen fachlich zulässigen Kontext wird die Kalkulation jetzt intern aufgebaut."
        ) +
        `<section class="panel">
          <h3>Kalkulation wird gestartet</h3>
          <p data-calculation-bootstrap-status role="status">
            Gesellschaft, Region, Los, Portalzugang und Kalkulationsfreigabe werden geprüft …
          </p>
          <p class="muted">
            Es findet keine externe Angebotsabgabe und keine verbindliche Portalaktion statt.
          </p>
        </section>`;

      const status =
        document.querySelector("[data-calculation-bootstrap-status]");

      const job = await mutate(
        `/management-inbox/autopilot/${encodeURIComponent(s.tender)}/jobs`,
        "POST",
        {
          action_type: "RUN_FULL_PIPELINE",
      ...
      if (status)
        status.textContent =
          `Kalkulationslauf gestartet · Job ${job.job_id}. Verarbeitung läuft …`;
      ...
      for (let attempt = 1; attempt <= 60; attempt += 1) {
        ...
        try {
          ...
          if (
            ...
            (pollError.code &&
              pollError.code !== "calculation_not_found")
          )
            throw pollError;

          if (status)
            status.textContent =
              `Kalkulationslauf ${job.job_id} wird verarbeitet · Prüfung ${attempt}/60 …`;
        }
      }
      ...
      if (!d) {
        if (status)
          status.textContent =
            `Der Kalkulationslauf ${job.job_id} benötigt länger. Bitte diese Ansicht erneut laden.`;

        return;
      }
    }

    const h = d.managementHeader,
...
<section class="panel calc-approval"><h3>Managementempfehlung</h3><p><strong>${esc(calcStatus(r.managementRecommendation))}</strong> · ${esc(r.recommendationReason || "Keine Empfehlung hinterlegt.")}</p><div class="calc-approval-summary">${calcMetric("Angebotspreis", calcMoney(q.totalPrice))}${calcMetric("DB1", calcMoney(q.db1))}${calcMetric("DB2", calcMoney(q.db2))}${calcMetric("DB3", calcMoney(q.db3))}${calcMetric("Gewinn", calcMoney(q.profit))}${calcMetric("Risiko", calcMoney(q.risk))}${calcMetric("Version", `Kalkulation ${h.calculationVersion}`)}${calcMetric("Angebotsfrist", calcDate(h.deadline))}</div><div class="review-actions"><button data-calc-decision="APPROVE" ${d.approval.eligible ? "" : "disabled"}>Kalkulation und Angebot freigeben</button><button data-calc-decision="REVISION_REQUESTED" ${d.approval.eligible ? "" : "disabled"}>Änderung anfordern</button><button data-calc-decision="REJECT" ${d.approval.eligible ? "" : "disabled"}>Ausschreibung ablehnen</button></div><p class="muted" data-calc-status aria-live="polite">Approval-Status: ${esc(calcStatus(d.approval.status))}. Externe Angebotsabgabe bleibt separat gesperrt.</p></section>`;
  ...
  const requiredDocumentGroup = (status,manuallyExcluded=false) =>
    manuallyExcluded ? "NICHT ERFORDERLICH" : status === "MISSING" || status === "REJECTED" ? "FEHLT" :
  ...
  async function openRequiredPdfEditor(s,id,selectedSourcePage) {
    const contextQuery=`company=${encodeURIComponent(s.company)}&lot=${encodeURIComponent(s.lot)}`,base=`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(id)}/working-copy`,data=await get(`${base}/fields?${contextQuery}`),item=data.item;
    const boundPage=Number(selectedSourcePage);if(!Number.isInteger(boundPage)||boundPage<1||Number(item.suggestedPage)!==boundPage)throw new Error("Die Seitenbindung der Anforderung hat sich geändert. Bitte Ansicht neu laden.");
    const control=field=>{
      if(!field.editable)return `<div class="pdf-form-field pdf-form-field-readonly"><strong>${esc(field.name)}</strong><span>${field.type==="signature"?"Signaturfeld – wird nicht automatisch ausgefüllt":"Nicht unterstützter Feldtyp"}</span></div>`;
      const name=esc(field.name),attr=`data-pdf-field="${name}" data-pdf-type="${esc(field.type)}"`;
      if(field.type==="checkbox")return `<label class="pdf-form-checkbox"><input type="checkbox" ${attr} ${field.value?"checked":""}>${name}</label>`;
      if(["radio","dropdown","option-list"].includes(field.type)){const values=Array.isArray(field.value)?field.value:[field.value],multiple=field.multiselect?" multiple":"";return `<label>${name}<select ${attr}${multiple}>${!field.multiselect?'<option value="">Nicht ausgefüllt</option>':""}${(field.options||[]).map(option=>`<option value="${esc(option)}" ${values.includes(option)?"selected":""}>${esc(option)}</option>`).join("")}</select></label>`}
      const max=field.maxLength?` maxlength="${Number(field.maxLength)}"`:"",input=field.multiline?`<textarea ${attr}${max}>${esc(field.value||"")}</textarea>`:`<input type="text" ${attr}${max} value="${esc(field.value||"")}">`;
      return `<label>${name}${input}</label>`;
    };
    const dialog=document.createElement("dialog");dialog.className="required-pdf-editor";dialog.innerHTML=`<form method="dialog" class="required-pdf-editor-shell"><header><div><h2>PDF am Bildschirm ausfüllen</h2><p>${esc(item.filename)} · Version <span data-pdf-version>${esc(item.version)}</span> · ${esc(item.pageCount)} Seiten</p></div><button value="close" aria-label="Editor schließen">Schließen</button></header><div class="required-pdf-editor-grid"><section class="required-pdf-preview"><div class="pdf-page-toolbar" role="toolbar" aria-label="PDF-Seitennavigation und Werkzeuge"><button type="button" data-page-prev aria-label="Vorherige Seite">←</button><label>Seite <input type="number" min="1" max="${esc(item.pageCount)}" value="1" data-page-number> / ${esc(item.pageCount)}</label><button type="button" data-page-next aria-label="Nächste Seite">→</button><span class="pdf-tool-separator"></span><button type="button" data-overlay-add="text">Textfeld</button><button type="button" data-overlay-add="checkbox">Checkbox</button><button type="button" data-overlay-add="mark" data-mark="x">X</button><button type="button" data-overlay-add="mark" data-mark="check">Haken</button><button type="button" data-overlay-add="note">Notiz</button><button type="button" data-overlay-add="mark" data-mark="highlight">Markierung</button></div><p class="muted" data-page-status aria-live="polite">PDF-Seite wird geladen …</p><div class="pdf-canvas-scroll"><div class="pdf-canvas-stage"><canvas data-pdf-canvas aria-label="PDF-Seitenvorschau"></canvas><div class="pdf-overlay-layer" data-overlay-layer aria-label="Frei positionierbare PDF-Elemente"></div></div></div></section><section class="required-pdf-fields"><h3>Overlay bearbeiten</h3><p>Text und Markierungen werden auf der gewählten Seite frei platziert. Positionen sind zoomunabhängig normalisiert.</p><div data-overlay-inspector><p class="muted">Werkzeug wählen oder ein Element auf der Seite anklicken.</p></div><h3>Vorhandene AcroForm-Felder</h3><p>${item.fields.length?"Vorhandene Formularfelder bleiben als Komfortpfad verfügbar.":"Dieses PDF hat keine AcroForm-Felder. Die universellen Overlay-Werkzeuge stehen trotzdem vollständig bereit."} Signaturen und rechtliche Bestätigungen werden nicht erzeugt.</p>${item.fields.map(control).join("")}<div class="review-actions"><button type="button" class="primary-action" data-pdf-save>Als neue Working-Copy-Version speichern</button><a class="button-link" data-working-copy-download href="${esc(`${API}${base}?${contextQuery}`)}" target="_blank" rel="noopener">Visuell ausgefüllte PDF herunterladen</a></div><p aria-live="polite" data-pdf-save-status>Original unverändert · keine externe Übertragung.</p></section></div></form>`;
    document.body.append(dialog);dialog.addEventListener("close",()=>dialog.remove());dialog.showModal();
    let elements=(item.overlays||[]).map(element=>({...element})),selectedId=null,currentPage=Math.min(Math.max(1,boundPage),item.pageCount),pdfDocument;
    const layer=dialog.querySelector("[data-overlay-layer]"),canvas=dialog.querySelector("[data-pdf-canvas]"),pageInput=dialog.querySelector("[data-page-number]"),pageStatus=dialog.querySelector("[data-page-status]"),inspector=dialog.querySelector("[data-overlay-inspector]");
    const labelFor=element=>element.type==="text"?(element.text||"Textfeld"):element.type==="note"?(element.text||"Notiz"):element.type==="checkbox"?`${element.checked?"☒":"☐"}`:({x:"×",check:"✓",yes:"JA",no:"NEIN",highlight:"Markierung"}[element.mark]||element.mark);
    const renderInspector=()=>{
      const element=elements.find(x=>x.id===selectedId);if(!element){inspector.innerHTML='<p class="muted">Werkzeug wählen oder ein Element auf der Seite anklicken.</p>';return}
      const text=["text","note"].includes(element.type)?`<label>Text<textarea data-overlay-text maxlength="2000">${esc(element.text||"")}</textarea></label><label>Schriftgröße (% der Seitenhöhe)<input type="number" data-overlay-font-size min="0.3" max="20" step="0.1" value="${esc((element.fontSize*100).toFixed(1))}"></label>`:"",kind=element.type==="checkbox"?`<label><input type="checkbox" data-overlay-checked ${element.checked?"checked":""}> Markiert</label><label>Zeichen<select data-overlay-mark><option value="x" ${element.mark==="x"?"selected":""}>X</option><option value="check" ${element.mark==="check"?"selected":""}>Haken</option></select></label>`:element.type==="mark"?`<label>Markierung<select data-overlay-mark>${[["x","X"],["check","Haken"],["yes","Ja"],["no","Nein"],["highlight","Hervorhebung"]].map(([value,label])=>`<option value="${value}" ${element.mark===value?"selected":""}>${label}</option>`).join("")}</select></label>`:"";
      inspector.innerHTML=`<div class="pdf-overlay-inspector"><strong>${esc(element.type)} · Seite ${esc(element.page)}</strong>${text}${kind}<fieldset class="pdf-overlay-size-controls"><legend>Elementgröße</legend><label>Breite (% der Seite)<input type="number" data-overlay-width min="0.5" max="${esc(((1-element.x)*100).toFixed(1))}" step="0.1" value="${esc((element.width*100).toFixed(1))}"></label><label>Höhe (% der Seite)<input type="number" data-overlay-height min="0.5" max="${esc(((1-element.y)*100).toFixed(1))}" step="0.1" value="${esc((element.height*100).toFixed(1))}"></label></fieldset><p class="muted">Alternativ den Griff unten rechts ziehen. Größen und Positionen bleiben beim Zoomen unverändert.</p><label>Farbe<select data-overlay-color>${["black","blue","red"].map(value=>`<option value="${value}" ${element.color===value?"selected":""}>${value==="black"?"Schwarz":value==="blue"?"Blau":"Rot"}</option>`).join("")}</select></label><button type="button" data-overlay-delete>Element löschen</button></div>`;
      const update=()=>{const textInput=inspector.querySelector("[data-overlay-text]"),checked=inspector.querySelector("[data-overlay-checked]"),mark=inspector.querySelector("[data-overlay-mark]"),color=inspector.querySelector("[data-overlay-color]"),width=inspector.querySelector("[data-overlay-width]"),height=inspector.querySelector("[data-overlay-height]"),fontSize=inspector.querySelector("[data-overlay-font-size]");if(textInput)element.text=textInput.value;if(checked)element.checked=checked.checked;if(mark)element.mark=mark.value;if(color)element.color=color.value;if(width)element.width=Math.max(.005,Math.min(1-element.x,Number(width.value)/100||element.width));if(height)element.height=Math.max(.005,Math.min(1-element.y,Number(height.value)/100||element.height));if(fontSize)element.fontSize=Math.max(.003,Math.min(.2,Number(fontSize.value)/100||element.fontSize));renderOverlays()};
      inspector.querySelectorAll("textarea,input,select").forEach(input=>input.oninput=update);inspector.querySelector("[data-overlay-delete]").onclick=()=>{elements=elements.filter(x=>x.id!==element.id);selectedId=null;renderOverlays();renderInspector()};
    };
    const renderOverlays=()=>{layer.innerHTML="";for(const element of elements.filter(x=>x.page===currentPage)){const node=document.createElement("button");node.type="button";node.className=`pdf-overlay-element pdf-overlay-${element.type}${element.id===selectedId?" selected":""}`;node.dataset.overlayId=element.id;node.setAttribute("aria-label",`${element.type} auf Seite ${element.page}; verschiebbar und größenveränderbar`);Object.assign(node.style,{left:`${element.x*100}%`,top:`${element.y*100}%`,width:`${element.width*100}%`,height:`${element.height*100}%`,fontSize:["text","note"].includes(element.type)?`${element.fontSize*layer.clientHeight}px`:`${Math.max(3,Math.min(element.width*layer.clientWidth,element.height*layer.clientHeight)*.7)}px`});node.textContent=labelFor(element);if(element.id===selectedId){const handle=document.createElement("span");handle.className="pdf-overlay-resize-handle";handle.dataset.overlayResize="true";handle.setAttribute("aria-hidden","true");node.append(handle)}node.onclick=()=>{selectedId=element.id;renderOverlays();renderInspector()};node.onpointerdown=event=>{if(event.button!==0)return;event.preventDefault();selectedId=element.id;const resizing=Boolean(event.target.closest("[data-overlay-resize]")),rect=layer.getBoundingClientRect(),startX=event.clientX,startY=event.clientY,originX=element.x,originY=element.y,originWidth=element.width,originHeight=element.height,moveHandler=move=>{if(resizing){element.width=Math.max(.005,Math.min(1-element.x,originWidth+(move.clientX-startX)/rect.width));element.height=Math.max(.005,Math.min(1-element.y,originHeight+(move.clientY-startY)/rect.height));Object.assign(node.style,{width:`${element.width*100}%`,height:`${element.height*100}%`})}else{element.x=Math.max(0,Math.min(1-element.width,originX+(move.clientX-startX)/rect.width));element.y=Math.max(0,Math.min(1-element.height,originY+(move.clientY-startY)/rect.height));Object.assign(node.style,{left:`${element.x*100}%`,top:`${element.y*100}%`})}},upHandler=()=>{document.removeEventListener("pointermove",moveHandler);document.removeEventListener("pointerup",upHandler);renderOverlays();renderInspector()};document.addEventListener("pointermove",moveHandler);document.addEventListener("pointerup",upHandler,{once:true})};node.onkeydown=event=>{const step=event.shiftKey?.02:.005;if(["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(event.key)){event.preventDefault();if(event.altKey){if(event.key==="ArrowLeft")element.width=Math.max(.005,element.width-step);if(event.key==="ArrowRight")element.width=Math.min(1-element.x,element.width+step);if(event.key==="ArrowUp")element.height=Math.max(.005,element.height-step);if(event.key==="ArrowDown")element.height=Math.min(1-element.y,element.height+step)}else{if(event.key==="ArrowLeft")element.x=Math.max(0,element.x-step);if(event.key==="ArrowRight")element.x=Math.min(1-element.width,element.x+step);if(event.key==="ArrowUp")element.y=Math.max(0,element.y-step);if(event.key==="ArrowDown")element.y=Math.min(1-element.height,element.y+step)}renderOverlays();renderInspector()}else if(event.key==="Delete"){elements=elements.filter(x=>x.id!==element.id);selectedId=null;renderOverlays();renderInspector()}};layer.append(node)}};
    const showPage=async pageNumber=>{currentPage=Math.min(Math.max(1,Number(pageNumber)||1),item.pageCount);pageInput.value=currentPage;pageStatus.textContent=`Seite ${currentPage} von ${item.pageCount} wird gerendert …`;const page=await pdfDocument.getPage(currentPage),unscaled=page.getViewport({scale:1}),available=Math.max(320,dialog.querySelector(".pdf-canvas-scroll").clientWidth-24),scale=Math.min(2,available/unscaled.width),viewport=page.getViewport({scale}),ratio=window.devicePixelRatio||1;canvas.width=Math.floor(viewport.width*ratio);canvas.height=Math.floor(viewport.height*ratio);canvas.style.width=`${viewport.width}px`;canvas.style.height=`${viewport.height}px`;layer.style.width=`${viewport.width}px`;layer.style.height=`${viewport.height}px`;await page.render({canvasContext:canvas.getContext("2d"),viewport,transform:ratio===1?null:[ratio,0,0,ratio,0,0]}).promise;pageStatus.textContent=`Seite ${currentPage} von ${item.pageCount}`;renderOverlays()};
    dialog.querySelectorAll("[data-overlay-add]").forEach(button=>button.onclick=()=>{const type=button.dataset.overlayAdd,id=`overlay-${crypto.randomUUID()}`,element={id,type,page:currentPage,x:.1,y:.1,width:(type==="text"||type==="note") ? .3 : .04,height:type==="text" ? .04 : type==="note" ? .1 : .04,color:type==="note"?"blue":"black"};if(type==="text"||type==="note")Object.assign(element,{text:"",fontSize:.018});if(type==="checkbox")Object.assign(element,{checked:true,mark:"x"});if(type==="mark")element.mark=button.dataset.mark;elements.push(element);selectedId=id;renderOverlays();renderInspector();inspector.querySelector("textarea")?.focus()});
    pageInput.onchange=()=>showPage(pageInput.value);dialog.querySelector("[data-page-prev]").onclick=()=>showPage(currentPage-1);dialog.querySelector("[data-page-next]").onclick=()=>showPage(currentPage+1);
    try{if(typeof Map.prototype.getOrInsertComputed!=="function")Object.defineProperty(Map.prototype,"getOrInsertComputed",{configurable:true,writable:true,value:function(key,callback){if(this.has(key))return this.get(key);const value=callback(key);this.set(key,value);return value}});const pdfjs=await import(`${BASE}/pdfjs/pdf.mjs`);pdfjs.GlobalWorkerOptions.workerSrc=`${BASE}/pdfjs/pdf.worker.mjs`;const response=await fetch(`${API}${base}?${contextQuery}&version=${item.version}`,{credentials:"same-origin",cache:"no-store"});if(!response.ok)throw new Error(`HTTP ${response.status}`);pdfDocument=await pdfjs.getDocument({data:new Uint8Array(await response.arrayBuffer())}).promise;item.pageCount=pdfDocument.numPages;pageInput.max=item.pageCount;await showPage(currentPage)}catch(error){pageStatus.textContent=`PDF-Vorschau konnte nicht geladen werden: ${error.message}`}
    dialog.querySelector("[data-pdf-save]").onclick=async()=>{const button=dialog.querySelector("[data-pdf-save]"),status=dialog.querySelector("[data-pdf-save-status]"),fields={};for(const input of dialog.querySelectorAll("[data-pdf-field]")){if(input.dataset.pdfType==="checkbox")fields[input.dataset.pdfField]=input.checked;else if(input.multiple)fields[input.dataset.pdfField]=[...input.selectedOptions].map(option=>option.value);else fields[input.dataset.pdfField]=input.value}button.disabled=true;status.textContent="Neue versionierte und visuell gerenderte Arbeitskopie wird gespeichert …";try{const saved=await mutate(`${base}/overlays`,"POST",{company:s.company,lot:s.lot,baseVersion:Number(item.version),fields,elements});item.version=saved.item.version;dialog.querySelector("[data-pdf-version]").textContent=item.version;dialog.querySelector("[data-working-copy-download]").href=`${API}${base}?${contextQuery}&version=${item.version}`;status.textContent=`Version ${item.version} mit ${saved.elementCount} Overlay-Elementen gespeichert. ${saved.statusLabel} Original unverändert; keine Signatur und keine externe Übertragung.`;await (s.view==="management-output"?managementView(s):contextView(s))}catch(error){status.textContent=`Änderungen nicht gespeichert: ${error.message}`}finally{button.disabled=false}};
  }
  async function openRequiredOfficeEditor(s,id) {
    const contextQuery=`company=${encodeURIComponent(s.company)}&lot=${encodeURIComponent(s.lot)}`,base=`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(id)}/working-copy/office`,data=await get(`${base}/fields?${contextQuery}`),item=data.item;
    const control=field=>{const fieldId=esc(field.id),label=`${esc(field.label)}${field.required?' <span aria-hidden="true">*</span>':""}`,common=`data-office-field="${fieldId}" data-office-type="${esc(field.type)}" ${field.required?"required":""}`;
      if(field.type==="checkbox")return `<label class="office-form-checkbox"><input type="checkbox" ${common} ${field.value?"checked":""}>${label}</label>`;
      if(field.type==="select")return `<label>${label}<select ${common}><option value="">Bitte wählen</option>${(field.options||[]).map(option=>`<option value="${esc(option)}" ${String(field.value)===String(option)?"selected":""}>${esc(option)}</option>`).join("")}</select></label>`;
      const type=field.type==="number"?"number":field.type==="date"?"date":"text",step=type==="number"?' step="any"':"";return `<label>${label}<input type="${type}" ${common}${step} value="${esc(field.value||"")}"></label>`};
    const sourceLabel=field=>field.source?.address?`${field.source.sheet} · ${field.source.address}`:field.source?.tag||`Feld ${field.source?.index||""}`;
    const dialog=document.createElement("dialog");dialog.className="required-office-editor";dialog.innerHTML=`<form class="required-office-editor-shell"><header><div><h2>${esc(item.format)} am Bildschirm ausfüllen</h2><p>${esc(item.filename)} · Version <span data-office-version>${esc(item.version)}</span> · ${esc(item.structuredFieldCount)} eindeutig strukturierte Felder</p></div><button type="button" data-office-close>Schließen</button></header><div class="required-office-editor-grid"><section class="required-office-fields"><h3>Formularfelder</h3><p>Pflichtfelder sind mit * gekennzeichnet. Änderungen werden nach kurzer Eingabepause als neue interne Version gespeichert.</p>${item.fields.map(control).join("")}</section><section class="required-office-preview"><h3>Struktur- und Quellenkontrolle</h3><p>Die erzeugte Datei wird serverseitig erneut eingelesen. Die native visuelle Endkontrolle in Word/Excel bleibt vor der fachlichen Freigabe verpflichtend.</p><div class="office-preview-table"><table><thead><tr><th>Feld</th><th>Fundstelle</th><th>Typ</th></tr></thead><tbody>${item.fields.map(field=>`<tr><th>${esc(field.label)}</th><td>${esc(sourceLabel(field))}</td><td>${esc(field.type)}</td></tr>`).join("")}</tbody></table></div><div class="review-actions"><button type="button" class="primary-action" data-office-save>Jetzt versioniert speichern</button><a class="button-link" data-office-download href="${esc(`${API}${base}?${contextQuery}&version=${item.version}`)}">Arbeitskopie zur visuellen Kontrolle herunterladen</a></div><p aria-live="polite" data-office-save-status>Original unverändert · serverseitiges Wieder­einlesen aktiv · keine externe Übertragung.</p></section></div></form>`;
    document.body.append(dialog);dialog.showModal();let dirty=false,saving=false,pending=false,timer=null;
    const status=dialog.querySelector("[data-office-save-status]"),saveButton=dialog.querySelector("[data-office-save]"),values=()=>Object.fromEntries([...dialog.querySelectorAll("[data-office-field]")].map(input=>[input.dataset.officeField,input.dataset.officeType==="checkbox"?input.checked:input.value]));
    const save=async({automatic=false}={})=>{if(saving){pending=true;return}if(!dirty&&!automatic)return;saving=true;saveButton.disabled=true;status.textContent=automatic?"Änderungen werden intern autosicher gespeichert …":"Neue interne Arbeitskopie-Version wird gespeichert …";try{const saved=await mutate(`${base}/fields`,"POST",{company:s.company,lot:s.lot,baseVersion:Number(item.version),fields:values()});item.version=Number(saved.item.version);dirty=false;dialog.querySelector("[data-office-version]").textContent=item.version;dialog.querySelector("[data-office-download]").href=`${API}${base}?${contextQuery}&version=${item.version}`;status.textContent=saved.idempotent?`Version ${item.version} unverändert; serverseitig erneut geprüft.`:`Version ${item.version} gespeichert und vollständig wieder eingelesen. Visuelle/fachliche Prüfung bleibt erforderlich; keine externe Übertragung.`}catch(error){status.textContent=`Änderungen nicht gespeichert: ${error.message}`;dirty=true}finally{saving=false;saveButton.disabled=false;if(pending){pending=false;save({automatic:true})}}};
    dialog.querySelectorAll("[data-office-field]").forEach(input=>input.addEventListener("input",()=>{dirty=true;status.textContent="Nicht gespeicherte Änderung; Autosicherung wird vorbereitet …";clearTimeout(timer);timer=setTimeout(()=>save({automatic:true}),1200)}));
    saveButton.onclick=()=>{clearTimeout(timer);save()};dialog.querySelector("[data-office-close]").onclick=async()=>{clearTimeout(timer);if(dirty)await save();if(!dirty&&!saving)dialog.close()};dialog.addEventListener("close",()=>{clearTimeout(timer);dialog.remove()},{once:true});
  }
  async function appendRequiredDocuments(s,{management=false}={}) {
    ...
    const openRequirement=x=>x.mandatory&&x.submission_relevant&&x.manual_submission_relevance_override!==false&&!["VALIDATED","NOT_REQUIRED","SUPERSEDED"].includes(x.satisfaction_status),formStatus=x=>!["PDF_EDITOR","OFFICE_EDITOR"].includes(x.action_type)?"Kein ausfüllbares Bieterformular.":x.original_form?.status==="AMBIGUOUS_MAPPING"?"Separates Originalformular mehrdeutig – fachliche Prüfung erforderlich.":x.original_form?.status==="NO_PROVEN_MAPPING"?"Kein separates Originalformular; nur eine exakt gebundene PDF-Fundstelle kann bearbeitet werden.":x.original_form?.status==="SUPERSEDED"?"Anforderung ersetzt; keine aktive Uploadpflicht.":"Ausfüllbares Bieterformular eindeutig zur Fundstelle gebunden.",
      card=(x)=>{const assignmentActive=x.mandatory&&x.submission_relevant&&!["NOT_REQUIRED","SUPERSEDED"].includes(x.satisfaction_status),manuallyExcluded=x.manual_submission_relevance_override===false,active=assignmentActive&&!manuallyExcluded,baseUrl=`${API}/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(x.id)}`,contextQuery=`company=${encodeURIComponent(s.company)}&lot=${encodeURIComponent(s.lot)}`,originalUrl=`${baseUrl}/original?${contextQuery}`,sourceUrl=`${baseUrl}/source?${contextQuery}`,sourcePage=x.source_document?.mimeType==="application/pdf"&&x.source_document?.page?`#page=${encodeURIComponent(x.source_document.page)}`:"",pdfEditable=active&&x.action_type==="PDF_EDITOR"&&x.source_document?.available&&x.source_document?.mimeType==="application/pdf",officeEditable=active&&x.action_type==="OFFICE_EDITOR"&&x.original_form?.editable&&["application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"].includes(x.original_form?.mimeType),uploadOnly=active&&x.action_type==="UPLOAD",impact=manuallyExcluded?"Manuell als für die Angebotsabgabe nicht erforderlich bestätigt; kein Submission-Blocker.":["NOT_REQUIRED","SUPERSEDED"].includes(x.satisfaction_status)||!x.submission_relevant?"Keine Unterlage zur Angebotsabgabe erforderlich; kein Submission-Blocker.":["MISSING","REJECTED"].includes(x.satisfaction_status)?"Die erforderliche Nachweisdatei fehlt oder wurde abgelehnt; die Angebotsabgabe ist derzeit blockiert.":["UPLOADED_PENDING_VALIDATION","MANUAL_REVIEW_REQUIRED"].includes(x.satisfaction_status)?"Eine gespeicherte Datei liegt vor und ist nicht mehr fehlend. Die fachliche Prüfung bleibt vor der Angebotsabgabe erforderlich.":"Kein offener Dokumentblocker.";return `<article class="required-document-card" data-required-document="${esc(x.id)}"><header><div><h4>${esc(x.requirement_title)}</h4><span class="status-badge required-${esc(requiredDocumentGroup(x.satisfaction_status,manuallyExcluded).toLowerCase().replaceAll(" ","-"))}">${esc(x.status_label)}</span></div><strong>${active?(x.mandatory?"Pflichtunterlage":"Optional"):"Keine Angebotsunterlage"}</strong></header><dl><dt>Quelle</dt><dd>${esc(x.source_reference)}${x.source_page?`, Seite ${esc(x.source_page)}`:""}</dd>${x.source_document?.available?`<dt>Quelldokument</dt><dd>${esc(x.source_document.filename)} · Dokumentversion ${esc(x.source_document.documentVersion||"belegt")}</dd>`:""}<dt>Anforderung</dt><dd>${esc(x.requirement_description)}</dd><dt>Einordnung</dt><dd>${esc(x.classification_reason||"")}</dd><dt>Auswirkung</dt><dd>${impact}</dd><dt>Originalformular</dt><dd>${esc(formStatus(x))}${x.original_form?.downloadable?` · Dokumentversion ${esc(x.original_form.documentVersion||"belegt")}${x.original_form.page?`, Seite ${esc(x.original_form.page)}`:""}`:""}</dd>${active?`<dt>Frist</dt><dd>${esc(calcDate(x.valid_until)||"Mit Angebotsabgabe")}</dd>`:""}${x.filename?`<dt>Aktuelle Datei</dt><dd><a href="${esc(API+`/tenders/${s.tender}/required-documents/${x.id}/download`)}">${esc(x.filename)}</a> · Version ${esc(x.upload_version)} · ${esc(Math.ceil(Number(x.size_bytes)/1024))} KB</dd><dt>Prüfung</dt><dd>${esc(x.validation_summary||x.status_label)}</dd>`:""}</dl><div class="review-actions source-actions">${x.source_document?.available?`<a class="button-link" data-source-open="${esc(x.id)}" href="${esc(sourceUrl+sourcePage)}" target="_blank" rel="noopener">Quelle öffnen</a><a class="button-link" data-source-download="${esc(x.id)}" href="${esc(`${baseUrl}/source/download?${contextQuery}`)}">Quelldokument herunterladen</a>`:""}${assignmentActive?`<button type="button" data-required-relevance="${esc(x.id)}">${manuallyExcluded?"Erforderlichkeit erneut prüfen":"Für Angebotsabgabe nicht erforderlich"}</button>`:""}${x.original_form?.downloadable&&(pdfEditable||officeEditable)?`<a class="button-link" data-original-download="${esc(x.id)}" href="${esc(originalUrl)}">Originalformular herunterladen</a>`:""}${pdfEditable?`<button type="button" data-pdf-edit="${esc(x.id)}" data-pdf-page="${esc(x.source_page)}">PDF am Bildschirm ausfüllen</button>`:""}${officeEditable?`<button type="button" data-office-edit="${esc(x.id)}">${esc(x.original_form.mimeType.includes("wordprocessingml")?"Word":"Excel")} am Bildschirm ausfüllen</button>`:""}${uploadOnly?`<label class="button-link upload-label">Nachweis hochladen<input type="file" data-required-upload="${esc(x.id)}" accept="${esc((x.accepted_formats||[]).join(","))}"></label>`:""}${active&&x.satisfaction_status==="MANUAL_REVIEW_REQUIRED"?`<button type="button" data-required-review="${esc(x.id)}" data-decision="VALIDATED">Als passend bestätigen</button><button type="button" data-required-review="${esc(x.id)}" data-decision="REJECTED">Ablehnen</button>`:""}</div><p class="muted" data-required-status="${esc(x.id)}" aria-live="polite"></p></article>`};
    const content=management?`<h3>Noch erforderliche Unterlagen für die Angebotsabgabe</h3><p><strong>${esc(data.summary.blockers)} Unterlagen offen, davon ${esc(data.summary.missing)} fehlend und ${esc(data.summary.manualReview)} zur Prüfung</strong></p>${data.items.filter(x=>openRequirement(x)||x.manual_submission_relevance_override===false).map(card).join("")||"<p>Keine offenen Pflichtunterlagen.</p>"}`:`<h3>Angebotsunterlagen / fehlende Nachweise</h3><div class="required-summary"><span>${esc(data.summary.missing)} fehlt/abgelehnt</span><span>${esc(data.summary.manualReview)} zur Prüfung</span><span>${esc(data.summary.validated)} vollständig</span></div>${groups.map(g=>{const items=data.items.filter(x=>x.satisfaction_status!=="SUPERSEDED"&&requiredDocumentGroup(x.satisfaction_status,x.manual_submission_relevance_override===false)===g);return items.length?`<section class="required-group"><h4>${g}</h4>${items.map(card).join("")}</section>`:""}).join("")}<div class="free-document-upload"><h4>Weitere Angebotsunterlage hochladen</h4><label>Dokument zuordnen zu<select data-free-required-select><option value="">Bitte auswählen</option>${data.items.filter(x=>x.manual_submission_relevance_override!==false&&!["VALIDATED","NOT_REQUIRED","SUPERSEDED"].includes(x.satisfaction_status)).map(x=>`<option value="${esc(x.id)}">${esc(x.requirement_title)}</option>`).join("")}<option value="OTHER">Sonstige Angebotsunterlage (erfüllt keinen Pflichtnachweis)</option></select></label><label class="button-link upload-label">Datei auswählen<input type="file" data-free-required-upload></label><p class="muted" data-free-upload-status></p></div>`;
    ...
    document.querySelectorAll("[data-required-relevance]").forEach(button=>button.onclick=()=>{const id=button.dataset.requiredRelevance,item=data.items.find(x=>x.id===id),wasExcluded=item?.manual_submission_relevance_override===false;document.querySelector("#wb-required-relevance-dialog")?.remove();const dialog=document.createElement("dialog");dialog.id="wb-required-relevance-dialog";dialog.className="approval-dialog";dialog.setAttribute("aria-labelledby","required-relevance-title");dialog.innerHTML=`<form method="dialog"><h2 id="required-relevance-title">Erforderlichkeit für die Angebotsabgabe</h2><p><strong>Ist dieser Nachweis für die Angebotsabgabe erforderlich?</strong></p><p>${wasExcluded?"Aktuell ist die Anforderung manuell von den Abgabeblockern ausgenommen. Mit Ja wird sie wieder als erforderlich behandelt.":"Nein nimmt nur diese konkrete Anforderung manuell aus den Abgabeblockern aus. Es wird kein Dokument als geprüft oder validiert markiert."}</p><div class="review-actions"><button type="button" data-relevance-answer="REQUIRED">Ja</button><button type="button" data-relevance-answer="NOT_REQUIRED">Nein</button><button type="button" data-relevance-cancel>Abbrechen</button></div><p class="error" data-relevance-error hidden></p></form>`;document.body.append(dialog);dialog.querySelector("[data-relevance-cancel]").onclick=()=>dialog.close();dialog.querySelectorAll("[data-relevance-answer]").forEach(answer=>answer.onclick=async()=>{const decision=answer.dataset.relevanceAnswer,error=dialog.querySelector("[data-relevance-error]");dialog.querySelectorAll("button").forEach(x=>x.disabled=true);try{const result=await mutate(`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(id)}/submission-relevance`,"POST",{company:s.company,lot:s.lot,decision});dialog.close();await rerender();const status=document.querySelector(`[data-required-status="${id}"]`);if(status)status.textContent=result.bidSubmissionRelevanceState==="MANUALLY_NOT_REQUIRED"?"Manuell als für die Angebotsabgabe nicht erforderlich bestätigt.":"Der Nachweis bleibt für die Angebotsabgabe erforderlich."}catch(ex){error.hidden=false;error.textContent=`Entscheidung nicht gespeichert: ${ex.message}`;dialog.querySelectorAll("button").forEach(x=>x.disabled=false)}});dialog.addEventListener("close",()=>dialog.remove(),{once:true});dialog.showModal()});
    document.querySelectorAll("[data-required-review]").forEach(button=>button.onclick=async()=>{const reason=prompt(button.dataset.decision==="VALIDATED"?"Prüfvermerk zur bestätigten Übereinstimmung":"Ablehnungsgrund");if(!reason)return;const item=data.items.find(x=>x.id===button.dataset.requiredReview),workingCopyTarget=item?.working_copy_id&&(!item.upload_id||["REQUIRED_PDF_WORKING_COPY","REQUIRED_OFFICE_WORKING_COPY"].includes(item.upload_source_type)),target=workingCopyTarget?{type:"WORKING_COPY",id:item.working_copy_id,version:Number(item.working_copy_version),sha256:item.working_copy_sha256,sourceDocumentId:item.working_source_document_id,sourceSha256:item.working_source_sha256}:item?.upload_id?{type:"UPLOAD",id:item.upload_id,version:Number(item.upload_version),sha256:item.sha256}:null,status=document.querySelector(`[data-required-status="${button.dataset.requiredReview}"]`);try{const r=await mutate(`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(button.dataset.requiredReview)}/review`,"POST",{company:s.company,lot:s.lot,decision:button.dataset.decision,reason,target});status.textContent=r.statusLabel;await rerender()}catch(error){status.textContent=`Prüfung nicht gespeichert: ${error.message}`}});
    ...
    document.querySelectorAll("[data-pdf-edit]").forEach(button=>button.onclick=async()=>{if(button.disabled)return;const id=button.dataset.pdfEdit,sourcePage=Number(button.dataset.pdfPage),status=document.querySelector(`[data-required-status="${id}"]`);button.disabled=true;button.setAttribute("aria-busy","true");status.textContent="Versionierte PDF-Arbeitskopie wird vorbereitet …";try{const result=await mutate(`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(id)}/working-copy`,"POST",{company:s.company,lot:s.lot});status.textContent=`Arbeitskopie Version ${result.item.version} ist bereit.`;await openRequiredPdfEditor(s,id,sourcePage)}catch(error){status.textContent=`Arbeitskopie konnte nicht vorbereitet werden: ${error.message}`}finally{button.disabled=false;button.removeAttribute("aria-busy")}});
    document.querySelectorAll("[data-office-edit]").forEach(button=>button.onclick=async()=>{if(button.disabled)return;const id=button.dataset.officeEdit,status=document.querySelector(`[data-required-status="${id}"]`);button.disabled=true;button.setAttribute("aria-busy","true");status.textContent="Versionierte Office-Arbeitskopie wird vorbereitet …";try{const result=await mutate(`/tenders/${encodeURIComponent(s.tender)}/required-documents/${encodeURIComponent(id)}/working-copy/office`,"POST",{company:s.company,lot:s.lot});status.textContent=`Arbeitskopie Version ${result.item.version} ist bereit.`;await openRequiredOfficeEditor(s,id)}catch(error){status.textContent=`Office-Arbeitskopie konnte nicht vorbereitet werden: ${error.message}`}finally{button.disabled=false;button.removeAttribute("aria-busy")}});
...
<details class="panel calc-technical"><summary>Revisionsnachweise</summary>${calcFacts(
  ...
  function portalCards(p) {
    const access = p.access || p.companyAccesses?.[0];
    if (!access) return "";
    const labels = { NOT_CONFIGURED:"Kein Portalzugang hinterlegt", CREDENTIAL_SCOPE_CONFLICT:"Gesellschaftszuordnung des Portalzugangs prüfen", CONFIGURED_UNVERIFIED:"Zugang gespeichert, noch nicht verifiziert", VALIDATION_PENDING:"Zugangsprüfung läuft", VALID:"Gültiger Portalzugang vorhanden", EXPIRED:"Portalzugang oder Sitzung abgelaufen", MFA_REQUIRED:"MFA-Bestätigung erforderlich", CAPTCHA_OR_USER_ACTION_REQUIRED:"Fortsetzung im Portal erforderlich", INVALID:"Portalzugang ungültig", LOCKED:"Portalzugang gesperrt", PORTAL_UNAVAILABLE:"Portal vorübergehend nicht erreichbar" },
      label = labels[access.status] || "Status unbekannt",
      tone = access.status === "VALID" ? "green" : ["INVALID","LOCKED","PORTAL_UNAVAILABLE"].includes(access.status) ? "red" : "yellow";
    const capabilities=new Set(p.serviceCapabilities||[]),canManageAccount=p.canManage&&!p.catalogVirtual&&(p.credentialAccountTypes||[]).length>0;
    const manageLabel = access.status === "NOT_CONFIGURED" ? "Zugangsdaten hinterlegen" : access.status === "CREDENTIAL_SCOPE_CONFLICT" ? "Zuordnung prüfen" : ["EXPIRED","INVALID"].includes(access.status) ? "Zugang aktualisieren" : "Zugangsdaten verwalten";
    return `<article class="panel portal-card" data-portal-card="${esc(p.portalId)}" data-portal-company="${esc(access.companyId)}"><header><div><h3>${esc(p.portalName)}</h3><p class="portal-company-name"><strong>${esc(access.companyName)}</strong></p></div><span class="status-badge status-${tone}">${esc(label)}</span></header><p><strong>${esc(p.domain)}</strong></p><p>${esc(p.purpose||"Zweck noch nicht klassifiziert")}</p><dl><dt>Betreiber</dt><dd>${esc(p.operator || p.portalName)}</dd><dt>Validierungsstatus</dt><dd>${esc(p.catalogValidationStatus||p.adapterValidationStatus)}</dd><dt>Login vorhanden</dt><dd>${yes(Boolean(p.loginAvailable||p.loginEntryUrl))}</dd><dt>Registrierung vorhanden</dt><dd>${yes(Boolean(p.registrationAvailable||p.registrationEntryUrl))}</dd><dt>Bekanntmachungssuche</dt><dd>${yes(capabilities.has("NOTICE_SEARCH"))}</dd><dt>Dokumentzugriff</dt><dd>${yes(["PUBLIC_DOCUMENT_ACCESS","AUTHENTICATED_DOCUMENT_ACCESS","TENDER_DOCUMENT_DOWNLOAD"].some(value=>capabilities.has(value)))}</dd><dt>Bekanntmachungsübermittlung</dt><dd>${yes(capabilities.has("NOTICE_PUBLICATION"))}</dd><dt>Bieterabgabe</dt><dd>${yes(capabilities.has("BID_SUBMISSION"))}</dd><dt>Rollen</dt><dd>${esc((p.serviceRoles||[]).join(", ")||"Noch zu prüfen")}</dd><dt>Zugang für ${esc(access.companyName)}</dt><dd>${access.configured ? "Ja" : "Nein"}</dd><dt>Zugangsstatus</dt><dd>${esc(label)}</dd><dt>Letzte erfolgreiche Prüfung</dt><dd>${esc(calcDate(access.lastSuccessfulCheck))}</dd></dl>${p.isTedService?'<p class="notice">Das tatsächliche Vergabe- und Abgabeportal dieser Ausschreibung kann von TED abweichen.</p>':""}<div class="review-actions">${p.openUrl?`<a class="button-link" href="${esc(p.openUrl)}" target="_blank" rel="noopener noreferrer">${p.domain==="ted.europa.eu"?"TED öffnen":"Dienst öffnen"}</a>`:""}${p.noticeSearchUrl?`<a class="button-link" href="${esc(p.noticeSearchUrl)}" target="_blank" rel="noopener noreferrer">Bekanntmachungen suchen</a>`:""}${canManageAccount ? `<button type="button" data-configure-portal="${esc(p.portalId)}" data-company="${esc(access.companyId)}">${esc(manageLabel)}</button>${access.configured&&access.authorizedCapabilities?.includes("BIDDER_LOGIN") && !["CREDENTIAL_SCOPE_CONFLICT","VALIDATION_PENDING"].includes(access.status) ? `<button type="button" data-test-portal="${esc(p.portalId)}" data-company="${esc(access.companyId)}" ${p.adapterEnabled ? "" : "disabled"}>Zugang prüfen</button>` : ""}${access.configured && access.status !== "CREDENTIAL_SCOPE_CONFLICT" && access.authorizedCapabilities?.includes("TENDER_DOCUMENT_DOWNLOAD") ? `<button type="button" data-test-documents="${esc(p.portalId)}" data-company="${esc(access.companyId)}" ${p.adapterEnabled ? "" : "disabled"}>Dokumentenabruf prüfen</button>` : ""}` : ""}</div><p class="muted" data-portal-status aria-live="polite">${esc(label)} · ${esc(access.companyName)}</p></article>`;
  }
  ...
  async function portalForm(portal, companyId, { locked = false } = {}) {
    if(portal.credentialEligible===false||(locked&&portal.tenderPortalSelectable===false)){
      const context=state(),params=new URLSearchParams({companyId:String(companyId),tenderId:String(context.tender||context.ausschreibung||""),mode:"search",returnTo:new URL(location.href).searchParams.get("returnTo")||"/admin/ausschreibungen"});
      out.innerHTML=title("Vergabeportal auswählen",portal.portalName)+`<section class="panel"><p>${portal.isTedService?"TED ist hier als Bekanntmachungsdienst sichtbar. ":""}Das für Anmeldung und Abgabe verwendete Vergabeportal konnte nicht eindeutig ermittelt werden.</p>${portal.openUrl?`<a class="button-link" href="${esc(portal.openUrl)}" target="_blank" rel="noopener noreferrer">TED öffnen</a>`:""}<a class="button-link" href="/admin/ausschreibungen/portalzugaenge?${params}">Tatsächliches Vergabeportal suchen und auswählen</a></section>`;
      return;
    }
    ...
    const credentialMetadata = scope.credential || {};
    out.innerHTML =
      title(
        scope.configured ? "Portalzugang bearbeiten" : "Portalzugang einrichten",
        `${portal.portalName} · ${company.name}`,
      ) +
      `<form id="portal-registry-links-form" class="panel" autocomplete="off"><h3>Offizielle Portalziele</h3><p class="muted">Nur administrativ geprüfte HTTPS-Ziele auf dem kanonischen oder freigegebenen Portalhost speichern. Keine Links anhand eines Portalnamens erraten.</p><label>Offizielle Loginseite<input name="loginUrl" type="url" inputmode="url" placeholder="https://…" value="${esc(portal.loginEntryUrl || "")}"></label><label>Offizielle Registrierungsseite<input name="registrationUrl" type="url" inputmode="url" placeholder="https://…" value="${esc(portal.registrationEntryUrl || "")}"></label><label><input name="verifiedOfficialLinks" type="checkbox" required> Ich habe Host und Ziel der offiziellen Seiten geprüft.</label><div class="review-actions"><button type="submit">Offizielle Portalziele verifizieren</button></div><p data-registry-links-status aria-live="polite">${portal.entryLinksVerifiedAt ? `Zuletzt verifiziert: ${esc(calcDate(portal.entryLinksVerifiedAt))}` : "Login- und Registrierungsziele noch nicht administrativ verifiziert."}</p></form>` +
      (scope.configured ? `<form id="portal-access-metadata-form" class="panel" autocomplete="off"><h3>Zugangsmetadaten</h3><p class="muted">Keine Passwörter, TOTP-Seeds, MFA-Codes oder Wiederherstellungscodes eintragen.</p><label>Interner Kontoname<input name="internalLabel" maxlength="120" value="${esc(credentialMetadata.internalLabel || "")}"></label><label>Verantwortliche Person<input name="contactPerson" maxlength="160" value="${esc(credentialMetadata.contactPerson || "")}"></label><label>Registrierungsstatus<select name="registrationStatus">${["NICHT_REGISTRIERT","REGISTRIERUNG_OFFEN","REGISTRIERT","MANUELLE_PRUEFUNG"].map(value=>`<option ${credentialMetadata.registrationStatus===value?"selected":""}>${value}</option>`).join("")}</select></label><label>Loginstatus<select name="loginStatus">${["LOGIN_UNGEPRUEFT","LOGIN_BESTAETIGT","MFA_ERFORDERLICH","ZUGANG_GESPERRT","ZUGANG_ABGELAUFEN","MANUELLE_PRUEFUNG"].map(value=>`<option ${credentialMetadata.loginStatus===value?"selected":""}>${value}</option>`).join("")}</select></label><label>MFA erforderlich<select name="mfaRequired"><option value="" ${credentialMetadata.mfaRequired==null?"selected":""}>Unbekannt</option><option value="true" ${credentialMetadata.mfaRequired===true?"selected":""}>Ja</option><option value="false" ${credentialMetadata.mfaRequired===false?"selected":""}>Nein</option></select></label><label>Hinweise ohne Geheimnisse<textarea name="notes" maxlength="1000">${esc(credentialMetadata.notes || "")}</textarea></label><label><input name="manualCheckConfirmed" type="checkbox"> Letzte erfolgreiche manuelle Prüfung auf jetzt setzen</label><div class="review-actions"><button type="submit">Zugangsmetadaten speichern</button></div><p data-access-metadata-status aria-live="polite">Letzte manuelle Prüfung: ${esc(calcDate(credentialMetadata.lastManualCheckAt))}</p></form>` : "") +
      `<form id="portal-credential-form" class="panel" autocomplete="off"><section class="portal-bound-scope" aria-label="Fest gebundener Zugangskontext"><h3>${esc(company.name)}</h3><dl><dt>Gesellschaft</dt><dd><strong>${esc(company.name)}</strong></dd><dt>Portal</dt><dd>${esc(portal.portalName)} · ${esc(portal.domain)}</dd><dt>Bindung</dt><dd>${locked ? "Aus dem Tender-Kontext fest vorgegeben" : "Für diese Zugangskarte fest ausgewählt"}</dd></dl><input type="hidden" name="companyId" value="${esc(company.id)}"></section>${scope.configured ? `<p class="muted">Gespeicherter Benutzername für <strong>${esc(company.name)}</strong>: ${esc(scope.credential.usernameMasked || "sicher maskiert")}. Das vorhandene Passwort wird niemals angezeigt oder zurückgeliefert. Zum Ändern neue Zugangsdaten eingeben.</p>` : `<p class="muted">Für ${esc(company.name)} ist an diesem Portal noch kein Zugang hinterlegt.</p>`}<label>Kontotyp<select name="accountType" required>${(portal.credentialAccountTypes||[]).map(value=>`<option value="${esc(value)}" ${credentialMetadata.accountType===value?"selected":""}>${esc(value)}</option>`).join("")}</select></label><fieldset><legend>Freigegebene Fähigkeiten dieses Kontos</legend>${(portal.serviceCapabilities?.length?portal.serviceCapabilities:["BIDDER_LOGIN","TENDER_DOCUMENT_DOWNLOAD","BID_SUBMISSION"]).map(value=>`<label><input type="checkbox" name="authorizedCapabilities" value="${esc(value)}" ${(credentialMetadata.authorizedCapabilities||[]).includes(value)?"checked":""}> ${esc(value)}</label>`).join("")}</fieldset><label>Benutzername oder E-Mail-Adresse<input name="username" required autocomplete="off" aria-describedby="credential-company-context"></label><p id="credential-company-context" class="muted">Dieser Benutzername wird ausschließlich ${esc(company.name)} und dem exakten Host ${esc(portal.domain)} zugeordnet.</p><label>Passwort<input name="password" type="password" required autocomplete="new-password"></label><label>MFA-Art<select name="mfaMethod"><option value="">Nicht erforderlich / unbekannt</option><option>TOTP</option><option>SMS</option><option>E-Mail</option><option>Portal-App</option><option>Persönliche Bestätigung</option></select></label><label>Ansprechpartner<input name="contactPerson" maxlength="160"></label><label>Bemerkungen<textarea name="notes" maxlength="1000"></textarea></label><label>Interne Bezeichnung<input name="internalLabel"></label><label><input name="accountConfirmed" type="checkbox"> Konto von ${esc(company.name)} ist fachlich bestätigt</label><div class="review-actions"><button type="submit">Für ${esc(company.name)} verschlüsselt speichern</button><button type="button" id="portal-form-save-test">Speichern und Anmeldung prüfen</button>${scope.configured ? '<button type="button" id="portal-form-test">Gespeicherten Zugang prüfen</button>' : ""}<button type="button" id="portal-form-cancel">Abbrechen</button></div><p id="portal-form-status" aria-live="polite"></p></form>`;
    const form = document.querySelector("#portal-credential-form"),
      payload = () => {
        ...
        return {
          ...
          authorizedCapabilities: data.getAll("authorizedCapabilities"),
          accountConfirmed: data.get("accountConfirmed") === "on",
        };
      };
    const registryForm = document.querySelector("#portal-registry-links-form");
    registryForm.onsubmit = async (event) => {
      event.preventDefault();
      const data = new FormData(registryForm),
        status = registryForm.querySelector("[data-registry-links-status]");
      try {
        const saved = await mutate(`/portal-access/${encodeURIComponent(portal.portalId)}/registry-links`, "PATCH", {
          loginUrl: String(data.get("loginUrl") || "").trim() || null,
          registrationUrl: String(data.get("registrationUrl") || "").trim() || null,
          verifiedOfficialLinks: data.get("verifiedOfficialLinks") === "on",
        });
        status.textContent = `Offizielle Portalziele für ${saved.canonicalHost} verifiziert. Geheimnisse wurden nicht verarbeitet.`;
      } catch (error) {
        status.textContent = `Portalziele konnten nicht gespeichert werden: ${error.message}`;
      }
    };
    const metadataForm = document.querySelector("#portal-access-metadata-form");
    if (metadataForm) metadataForm.onsubmit = async (event) => {
      event.preventDefault();
      const data = new FormData(metadataForm), status = metadataForm.querySelector("[data-access-metadata-status]"), mfa = String(data.get("mfaRequired") || "");
      try {
        await mutate(`/portal-access/${encodeURIComponent(portal.portalId)}/credential-metadata`, "PATCH", {
          ...
          mfaRequired: mfa === "" ? null : mfa === "true",
          manualCheckConfirmed: data.get("manualCheckConfirmed") === "on",
        });
        status.textContent = `Zugangsmetadaten für ${company.name} gespeichert. Geheimniswerte wurden weder übertragen noch protokolliert.`;
      } catch (error) {
        status.textContent = `Zugangsmetadaten konnten nicht gespeichert werden: ${error.message}`;
      }
    };
    const testSavedCredential = async (status) => {
      const context=state(),tenderId=context.tender||context.ausschreibung;
      const expectedCredentialId=String(form.dataset.credentialId||""),expectedCredentialVersion=Number(form.dataset.credentialVersion||0)||null;
      const job = await mutate(
        ...
        "POST",
        {
          ...
          ...(context.lot?{lot_id:context.lot}:{}),
          ...(expectedCredentialId?{credential_id:expectedCredentialId}:{}),
          ...(expectedCredentialVersion?{credential_version:expectedCredentialVersion}:{}),
        },
      );
      form.dataset.credentialId=job.credential_id||expectedCredentialId;
      form.dataset.credentialVersion=job.credential_version||expectedCredentialVersion||"";
      status.textContent = `Prüfung läuft für Credential-Version ${job.credential_version || expectedCredentialVersion}. Aktueller Verarbeitungsschritt: ${humanizedEnum(job.current_step || job.status)}.`;
      status.dataset.portalStatus = portal.portalId;
      status.dataset.jobId = job.job_id;
      localStorage.setItem(`wb-tender-job:${job.job_id}`,JSON.stringify({portalId:portal.portalId,companyId:company.id}));
      trackJob(job.job_id);
      return job;
    };
    const saveCredential = async ({ testAfterSave = false } = {}) => {
      const status = document.querySelector("#portal-form-status"),buttons=[...form.querySelectorAll("button")],idempotencyKey=form.dataset.pendingIdempotencyKey||(form.dataset.pendingIdempotencyKey=crypto.randomUUID());
      ...
      try {
        ...
        form.dataset.credentialId=result.credentialId;
        form.dataset.credentialVersion=result.credentialVersion;
        form.dataset.credentialFingerprint=result.credentialFingerprint;
        status.textContent = `Zugang sicher gespeichert · Credential-Version ${result.credentialVersion}. Das Passwort wird nicht wieder angezeigt.`;
        if (testAfterSave) {
          try { await testSavedCredential(status); }
          catch (error) {
            status.dataset.code=error.code==="CREDENTIAL_VERSION_CONFLICT"?"CREDENTIAL_VERSION_CONFLICT":"CREDENTIAL_GESPEICHERT_PRUEFUNG_NICHT_GESTARTET";
            status.textContent = error.code==="CREDENTIAL_VERSION_CONFLICT"
              ? `Der Portalzugang wurde sicher gespeichert. Inzwischen ist Version ${error.currentCredentialVersion || "unbekannt"} aktuell. Bitte laden Sie die Ansicht neu; das Passwort wurde nicht erneut gesendet.`
              : "Der Portalzugang wurde sicher gespeichert. Die Anmeldeprüfung konnte noch nicht gestartet werden.";
          }
        }
      } catch (error) {
        if(error.code==="CREDENTIAL_VERSION_CONFLICT")form.elements.password.value="";
    ...
    if (testButton)
      testButton.onclick = async () => {
        ...
        try{await testSavedCredential(document.querySelector("#portal-form-status"))}
        catch(error){if(error.code==="CREDENTIAL_VERSION_CONFLICT")form.elements.password.value="";document.querySelector("#portal-form-status").textContent=`Anmeldung konnte nicht geprüft werden: ${error.message}`}
  ...
  const continuationTarget = (continuation) => {
    if (!continuation?.actionType) return null;
    const scoped = {
        ...state(),
        tender: continuation.tenderId || state().tender,
        company: continuation.companyId || state().company,
        lot: continuation.lotKey || "",
        enrichment: continuation.enrichmentVersionId || state().enrichment,
      },
      view = continuation.actionType === "REVIEW_TENDER_CONTEXT" ? "detail" : "portal-access",
      target = new URL(href(view, scoped), location.origin);
    if (continuation.portalId) target.searchParams.set("portal", continuation.portalId);
    if (["REVIEW_PORTAL_ADAPTER", "REVIEW_PORTAL_AVAILABILITY"].includes(continuation.actionType))
      target.searchParams.set("focus", "adapter");
    if (["MANAGE_PORTAL_ACCESS", "CONTINUE_PORTAL_AUTHENTICATION"].includes(continuation.actionType))
      target.searchParams.set("focus", "credentials");
    return target.pathname + target.search;
  };
  const appendContinuation = (node, continuation) => {
    const target = continuationTarget(continuation);
    if (!target) return;
    const box = document.createElement("span"),
      heading = document.createElement("strong"),
      message = document.createElement("span"),
      action = document.createElement("a");
    box.className = "job-continuation";
    box.setAttribute("role", "group");
    box.setAttribute("aria-label", continuation.title || "Erforderliche Fortsetzung");
    heading.textContent = continuation.title || "Erforderliche Fortsetzung";
    message.textContent = ` ${continuation.message || "Eine sichere Fortsetzung ist erforderlich."}`;
    action.className = "button-link job-continuation-action";
    action.href = target;
    action.textContent = continuation.actionLabel || "Fortsetzung öffnen";
    action.dataset.continuationType = continuation.actionType;
    box.append(heading, message, document.createElement("br"), action);
    node.append(document.createElement("br"), box);
  };
  ...
  function showJob(id, job, note = "") {
    const meta = jobMeta(id),
      ...
      text = job.continuation && terminal.has(job.status)
        ? `Job-ID ${id} · ${job.continuation.status} · externe Aktion: nein`
        : isLoginTest && terminal.has(job.status)
    ...
    jobTargets(id, meta).forEach((node) => {
      ...
      if (job.continuation && terminal.has(job.status)) appendContinuation(node, job.continuation);
    ...
    const state = {
      ...
      tick = async () => {
        ...
        try {
          let portalJob = null;
          try {
            portalJob = JSON.parse(localStorage.getItem(`wb-tender-job:${id}`) || "null");
          } catch {}
          const job = await get(
            portalJob?.portalId
              ? `/portal-access/jobs/${encodeURIComponent(id)}`
          ...
          if (terminal.has(job.status)) {
            ...
            if (portalJob?.portalId && state().view === "portal-access")
              setTimeout(() => portalView(), 0);
            return;
          }
          schedule(delay);
        } catch (error) {
          ...
          if (error.status === 404 || error.status === 410) {
            showJob(id, { status: "STATUS_UNKNOWN" }, `Job-ID ${id} · Statusabruf beendet: Job nicht vorhanden oder durch eine neuere Credential-Version ersetzt.`);
            stopJobPoller(id, { remove: true });
            return;
          }
  ...
  async function portalView() {
    ...
    for (const key of ["company","tender","service","version","lot","portal","q","access","status","validated","authentication","documentDownload","portalRole","page","pageSize","portalHost"])
      if (requestedParams.get(key)) apiParams.set(key, requestedParams.get(key));
    const d = await get(`/portals?${apiParams}`);
    if (!requestedParams.get("company") && d.selectedCompany?.id) {
      requestedParams.set("company", d.selectedCompany.id);
      history.replaceState(null, "", `${location.pathname}?${requestedParams}`);
    }
    const option = (value, label, key) => `<option value="${esc(value)}" ${requestedParams.get(key) === value ? "selected" : ""}>${esc(label)}</option>`,
      setQuery = (key, value) => {
    ...
    out.innerHTML =
      ...
      `<section class="panel portal-search-panel"><label for="portal-search"><strong>Vergabeportal oder Anbieter suchen</strong></label><div class="portal-search-row"><input id="portal-search" type="search" value="${esc(requestedParams.get("q") || "")}" placeholder="Portalname, Betreiber, Domain oder Alias eingeben" autocomplete="off"><button type="button" id="portal-search-reset">Suche zurücksetzen</button></div><div class="portal-filter-grid"><label>Gesellschaft<select id="portal-company-filter" ${d.companyLocked ? "disabled" : ""}>${d.companies.map(company => option(company.id, company.name, "company")).join("")}</select></label><label>Diensttyp<select data-portal-filter="portalRole">${option("","Alle Portale","portalRole")}${option("publication","Veröffentlichungsportale","portalRole")}${option("notices","Bekanntmachungsportale","portalRole")}${option("documents","Dokumentportale","portalRole")}${option("login","Loginportale","portalRole")}${option("submission","Abgabeportale","portalRole")}${option("ted","TED-Dienste","portalRole")}</select></label><label>Zugang<select data-portal-filter="access">${option("","Alle","access")}${option("present","Zugang vorhanden","access")}${option("missing","Zugang fehlt","access")}</select></label><label>Zugangsstatus<select data-portal-filter="status">${option("","Alle","status")}${option("VALID","Gültig","status")}${option("CREDENTIAL_SCOPE_CONFLICT","Gesellschaftszuordnung prüfen","status")}${option("CONFIGURED_UNVERIFIED","Gespeichert, ungeprüft","status")}${option("EXPIRED","Abgelaufen","status")}${option("MFA_REQUIRED","MFA erforderlich","status")}${option("CAPTCHA_OR_USER_ACTION_REQUIRED","Fortsetzung erforderlich","status")}${option("VALIDATION_PENDING","Prüfung läuft","status")}${option("INVALID","Ungültig","status")}${option("LOCKED","Gesperrt","status")}${option("PORTAL_UNAVAILABLE","Portal nicht erreichbar","status")}${option("NOT_CONFIGURED","Nicht konfiguriert","status")}</select></label><label>Portalvalidierung<select data-portal-filter="validated">${option("","Alle","validated")}${option("true","Portal validiert","validated")}${option("false","Portal noch zu prüfen","validated")}</select></label><label>Authentifizierung<select data-portal-filter="authentication">${option("","Alle","authentication")}${option("true","Unterstützt","authentication")}${option("false","Nicht unterstützt","authentication")}</select></label><label>Dokumentdownload<select data-portal-filter="documentDownload">${option("","Alle","documentDownload")}${option("true","Unterstützt","documentDownload")}${option("false","Nicht unterstützt","documentDownload")}</select></label><label>Treffer pro Seite<select data-portal-filter="pageSize">${option("25","25","pageSize")}${option("50","50","pageSize")}</select></label></div><p><strong>${esc(d.total)}</strong> Treffer</p></section>`+
      (d.items.length ? `<div class="grid portal-grid" data-portal-results>${d.items.map(portalCards).join("")}</div>` : `<section class="panel"><p><strong>Kein passendes Vergabeportal gefunden.</strong></p><div class="review-actions"><button type="button" id="portal-show-all">Alle Portale anzeigen</button><button type="button" id="portal-candidate">Portal als Prüfungskandidat erfassen</button></div><p class="muted">Ein Prüfungskandidat wird nicht automatisch validiert.</p></section>`)+
      `<nav class="portal-pagination" aria-label="Suchergebnisse"><button type="button" data-portal-page="${d.page - 1}" ${d.page <= 1 ? "disabled" : ""}>Zurück</button><span>Seite ${esc(d.page)} von ${esc(d.pages)}</span><button type="button" data-portal-page="${d.page + 1}" ${d.page >= d.pages ? "disabled" : ""}>Weiter</button></nav>`;
    let searchTimer;
    const searchInput = document.querySelector("#portal-search");
    searchInput.oninput = () => {
      clearTimeout(searchTimer);
      searchTimer = setTimeout(() => { setQuery("q", searchInput.value.trim()); portalView(); }, 300);
    };
    searchInput.onkeydown = (event) => {
      if (event.key === "Escape") { event.preventDefault(); searchInput.value = ""; setQuery("q", ""); portalView(); }
      if (event.key === "Enter" && d.items[0]) { event.preventDefault(); portalForm(d.items[0], d.selectedCompany.id, { locked: d.companyLocked }); }
    };
    document.querySelector("#portal-search-reset").onclick = () => { setQuery("q", ""); portalView(); };
    document.querySelector("#portal-show-all")?.addEventListener("click", () => { setQuery("q", ""); portalView(); });
    document.querySelector("#portal-candidate")?.addEventListener("click", async button => {
      const candidate = searchInput.value.trim();
      if (!candidate) return searchInput.focus();
      button.currentTarget.disabled = true;
      try {
        await mutate("/portal-access/registry-candidates", "POST", { candidate, companyId:d.selectedCompany.id, tenderId:state().tender || null });
        button.currentTarget.textContent = "Als Prüfungskandidat erfasst";
      } catch (error) {
        button.currentTarget.disabled = false;
        button.currentTarget.textContent = error.message;
      }
    });
    document.querySelector("#portal-company-filter").onchange = (event) => { setQuery("company", event.target.value); portalView(); };
    document.querySelectorAll("[data-portal-filter]").forEach(select => select.onchange = () => { setQuery(select.dataset.portalFilter, select.value); portalView(); });
    document.querySelectorAll("[data-portal-page]").forEach(button => button.onclick = () => { setQuery("page", button.dataset.portalPage); portalView(); });
    if (requestedParams.get("mode") === "search") searchInput.focus();
    ...
    document
      ...
      .forEach(
        (b) => (b.onclick = () => portalForm(byId(b.dataset.configurePortal), b.dataset.company, { locked: d.companyLocked })),
      );
  ...
  async function globalView(s) {
    ...
    if(s.view==="company-profiles"){
      const d=await get("/profiles"),status={READY_FOR_APPROVAL:"Freigabefähig",ACTIVE:"Aktiv",DRAFT:"Entwurf",SUPERSEDED:"Durch neuere Version ersetzt",REVOKED:"Widerrufen"},stateLabel={VALUE_MISSING:"Wert fehlt",SOURCE_MISSING:"Quelle fehlt",VALUE_AND_SOURCE_MISSING:"Wert und Quelle fehlen",EVIDENCE_EXPIRED:"Nachweis abgelaufen",SOURCE_NOT_AUTHORITATIVE:"Quelle nicht autoritativ",VALIDATION_FAILED:"Validierung fehlgeschlagen",NOT_APPLICABLE_REASON_MISSING:"Nicht zutreffend – Begründung fehlt",COMPLETE:"Vollständig"};
      const fieldRows=x=>x.fields.map(f=>`<tr class="profile-field-${esc(f.state.toLowerCase())}"><th scope="row">${esc(f.label)}<small>${esc(f.area)}</small></th><td><strong>${esc(stateLabel[f.state]||f.state)}</strong><br>${esc(f.reason)}</td><td>${esc(f.source.label)}<br><small>Zuständig: ${esc(f.role)}</small></td><td>${f.complete?"Vollständig":x.editable?`<button type="button" data-profile-field="${esc(f.key)}" data-profile-id="${esc(x.id)}">${esc(f.action)}</button>`:"Nur aktuelle Version bearbeitbar"}</td></tr>`).join("");
      const currentCard=x=>`<article class="panel company-profile-card" data-profile-card="${esc(x.id)}"><header><h2>${esc(x.legal_name||x.name)}</h2><p>${esc(x.service_line||x.capabilities?.serviceLine||"Nicht eindeutig belegt")} · Version ${esc(x.version)}</p></header><div class="profile-summary"><span><strong>${esc(x.completenessPercent)} %</strong> vollständig</span><span><strong>${esc(x.missingValues)}</strong> fehlende Werte</span><span><strong>${esc(x.missingSources)}</strong> fehlende/ungeprüfte Quellen</span><span><strong>${esc(status[x.lifecycle_status]||x.lifecycle_status)}</strong></span></div><p><strong>Nächster Schritt:</strong> ${esc(x.nextStep)}</p>${x.editable&&!x.releaseReady?`<button type="button" class="primary-action" data-profile-wizard="${esc(x.id)}">Pflichtfelder vervollständigen</button>`:""}${x.releaseReady?`<button type="button" data-profile-activate="${esc(x.id)}" data-profile-hash="${esc(x.profile_sha256)}">Vorhandenen Freigabeprozess öffnen</button>`:`<p><strong>Nicht freigabefähig:</strong> Pflichtfelder oder autoritative Quellen fehlen.</p>`}<div class="profile-fields-scroll"><table class="profile-fields"><thead><tr><th>Pflichtfeld</th><th>Zustand / Blockierungsgrund</th><th>Erforderlich</th><th>Aktion</th></tr></thead><tbody>${fieldRows(x)}</tbody></table></div><details><summary>Versionshistorie anzeigen (${esc((d.groups.find(g=>g.current?.id===x.id)?.history||[]).length)})</summary>${(d.groups.find(g=>g.current?.id===x.id)?.history||[]).map(h=>`<article class="profile-history"><h3>Version ${esc(h.version)} · ${esc(status[h.lifecycle_status]||h.lifecycle_status)}</h3><p>Unveränderlich · ${esc(h.completenessPercent)} % vollständig</p><button type="button" data-profile-details="${esc(h.id)}">Details anzeigen</button> <button type="button" data-profile-compare="${esc(h.id)}" data-current-id="${esc(x.id)}">Mit aktueller Version vergleichen</button></article>`).join("")||"<p>Keine historische Version.</p>"}</details></article>`;
      out.innerHTML=title("Gesellschaftsprofile","Aktuelle Ergänzungsversionen zuerst. Historische und freigegebene Versionen bleiben unveränderlich.")+`<section aria-label="Aktuelle Gesellschaftsprofile">${d.groups.map(g=>currentCard(g.current)).join("")}</section><dialog id="profile-field-dialog" class="profile-field-dialog"></dialog>`;
      const byId=Object.fromEntries(d.items.map(x=>[x.id,x])),dialog=out.querySelector("#profile-field-dialog");
      const openField=async(profile,field,wizard=false)=>{const value=field.value==null?"":typeof field.value==="string"?field.value:JSON.stringify(field.value,null,2),p=field.provenance||{},types=field.source.types.map(t=>`<option value="${esc(t)}" ${p.sourceType===t?"selected":""}>${esc(d.sourceTypes[t]||t)}</option>`).join("");dialog.innerHTML=`<form method="dialog" class="panel profile-field-form"><p class="wizard-progress">${wizard?`Schritt ${profile.fields.filter(x=>!x.complete).findIndex(x=>x.key===field.key)+1} von ${profile.fields.filter(x=>!x.complete).length}`:"Einzelbearbeitung"}</p><h2>${esc(field.label)}</h2><dl><dt>Gesellschaft</dt><dd>${esc(profile.legal_name)}</dd><dt>Leistungsbereich</dt><dd>${esc(profile.service_line)}</dd><dt>Bearbeitbare Profilversion</dt><dd>${esc(profile.version)}</dd><dt>Warum erforderlich</dt><dd>${esc(field.reason)}</dd><dt>Benötigte Quelle</dt><dd>${esc(field.source.label)}</dd></dl><label>Wert<textarea name="value" maxlength="10000">${esc(value)}</textarea></label><label>Einheit<input name="unit" value="${esc(p.unit||"")}"></label><label>Gültig ab<input type="date" name="validFrom" value="${esc(p.validFrom||"")}"></label><label>Gültig bis<input type="date" name="validUntil" value="${esc(p.validUntil||"")}"></label><label>Quellenart<select name="sourceType"><option value="">Bitte wählen</option>${types}</select></label><label>Quellenbezeichnung<input name="sourceLabel" value="${esc(p.sourceLabel||"")}"></label><label>Aussteller / verantwortliche Stelle<input name="issuer" value="${esc(p.issuer||"")}"></label><label>Ausstellungs-/Beschlussdatum<input type="date" name="issuedAt" value="${esc(p.issuedAt||"")}"></label><label>Dokument-/Referenznummer<input name="documentReference" value="${esc(p.documentReference||"")}"></label><label>Verifizierungsstatus<select name="verificationStatus"><option value="PENDING_REVIEW">Zur Prüfung vorgemerkt</option><option value="VERIFIED" ${p.verificationStatus==="VERIFIED"?"selected":""}>Als autorisierte Rolle verifizieren</option></select></label><label>Nachweisdatei (PDF, PNG oder JPEG; max. 10 MB)<input type="file" name="evidence" accept="application/pdf,image/png,image/jpeg"></label>${field.notApplicableAllowed?`<label><input type="checkbox" name="notApplicable" ${p.notApplicable?"checked":""}> Nicht zutreffend</label><label>Begründung für „Nicht zutreffend“<textarea name="notApplicableReason">${esc(p.notApplicableReason||"")}</textarea></label>`:""}<label>Änderungsbegründung<textarea required name="reason"></textarea></label><label>Interne Bemerkung<textarea name="internalNote">${esc(p.internalNote||"")}</textarea></label><p role="alert" class="error" data-profile-error hidden></p><div class="card-actions"><button type="submit" value="save" class="primary-action">${wizard?"Speichern und weiter":"Als Entwurf speichern"}</button>${wizard?'<button type="submit" value="later">Später fortsetzen</button>':""}<button type="submit" value="cancel">Abbrechen</button><a class="button-link" href="${esc(href("company-profiles",state()))}">Zurück zum Gesellschaftsprofil</a></div></form>`;dialog.showModal();dialog.querySelector("textarea[name=value]")?.focus();const form=dialog.querySelector("form");form.addEventListener("submit",async event=>{event.preventDefault();const action=event.submitter?.value;if(action==="cancel"||action==="later"){if(action==="later")sessionStorage.setItem(`profile-wizard:${profile.id}`,field.key);dialog.close();return}const f=new FormData(form),file=f.get("evidence"),error=form.querySelector("[data-profile-error]");let upload=null;if(file?.size)upload={filename:file.name,mediaType:file.type,base64:await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(",")[1]||"");reader.onerror=reject;reader.readAsDataURL(file)})};const sourceType=String(f.get("sourceType")||""),source=sourceType?{sourceType,sourceLabel:f.get("sourceLabel"),issuer:f.get("issuer"),issuedAt:f.get("issuedAt")||null,validFrom:f.get("validFrom")||null,validUntil:f.get("validUntil")||null,documentReference:f.get("documentReference"),verificationStatus:f.get("verificationStatus")}:null;let parsed=String(f.get("value")||"").trim();if((parsed.startsWith("[")&&parsed.endsWith("]"))||(parsed.startsWith("{")&&parsed.endsWith("}")))try{parsed=JSON.parse(parsed)}catch{}try{await mutate(`/profiles/${profile.id}/fields/${field.key}`,"PUT",{expectedProfileSha256:profile.profile_sha256,value:parsed||undefined,unit:f.get("unit"),source,upload,notApplicable:f.get("notApplicable")==="on",notApplicableReason:f.get("notApplicableReason"),reason:f.get("reason"),internalNote:f.get("internalNote")});dialog.close();if(wizard)sessionStorage.setItem(`profile-wizard:${profile.id}`,field.key);await render()}catch(e){error.hidden=false;error.textContent=`${e.message} Request-ID: ${e.requestId||"nicht verfügbar"}`;error.focus()}})};
      for(const button of out.querySelectorAll("[data-profile-field]"))button.addEventListener("click",()=>{const profile=byId[button.dataset.profileId],field=profile.fields.find(x=>x.key===button.dataset.profileField);openField(profile,field)});
      for(const button of out.querySelectorAll("[data-profile-wizard]"))button.addEventListener("click",()=>{const profile=byId[button.dataset.profileWizard],open=profile.fields.filter(x=>!x.complete),saved=sessionStorage.getItem(`profile-wizard:${profile.id}`),index=Math.max(0,open.findIndex(x=>x.key===saved)+1)%open.length;openField(profile,open[index],true)});
      for(const button of out.querySelectorAll("[data-profile-details]"))button.addEventListener("click",()=>{const x=byId[button.dataset.profileDetails];dialog.innerHTML=`<form method="dialog" class="panel"><h2>${esc(x.legal_name)} · Version ${esc(x.version)}</h2><p>Historische Version – unveränderlich.</p><div class="profile-fields-scroll"><table class="profile-fields"><tbody>${fieldRows(x)}</tbody></table></div><button>Schließen</button></form>`;dialog.showModal()});
      for(const button of out.querySelectorAll("[data-profile-compare]"))button.addEventListener("click",()=>{const old=byId[button.dataset.profileCompare],current=byId[button.dataset.currentId];dialog.innerHTML=`<form method="dialog" class="panel"><h2>Version ${esc(old.version)} mit Version ${esc(current.version)} vergleichen</h2><table><thead><tr><th>Pflichtfeld</th><th>Historisch</th><th>Aktuell</th></tr></thead><tbody>${current.fields.map(f=>{const previous=old.fields.find(x=>x.key===f.key);return `<tr><th>${esc(f.label)}</th><td>${esc(stateLabel[previous?.state]||previous?.state)}</td><td>${esc(stateLabel[f.state]||f.state)}</td></tr>`}).join("")}</tbody></table><button>Schließen</button></form>`;dialog.showModal()});
      for(const button of out.querySelectorAll("[data-profile-activate]"))button.addEventListener("click",async()=>{if(!confirm("Diese konkrete Profilversion und ihre angezeigten Quellen verbindlich freigeben?"))return;await mutate(`/profiles/${button.dataset.profileActivate}/activate`,"POST",{profileSha256:button.dataset.profileHash,confirmation:"Ich bestätige diese konkrete Gesellschaftsprofilversion und ihre Quellen."});await render()});return;
    }
    if(s.view==="regions"){
      const d=await get("/regions"),status={ACTIVE:"Aktiv",SUPERSEDED:"Durch neuere Version ersetzt",RETIRED:"Stillgelegt"},change={CREATE:"Angelegt",UPDATE:"Bearbeitet",RELOCATION:"Umzug",RETIRE:"Stillgelegt",LEGACY_IMPORT:"Übernommener Bestand"};
      const history=items=>items.map(x=>`<tr><td>${esc(x.version)}</td><td>${esc(status[x.lifecycle_status]||x.lifecycle_status)}</td><td>${esc(change[x.change_type]||x.change_type)}</td><td>${esc(calcDate(x.effective_from))}</td><td>${esc(x.change_reason)}</td><td><code>${esc(x.content_sha256)}</code></td></tr>`).join("");
      out.innerHTML=title("Regionale Zonen","Jede fachliche Änderung erzeugt eine unveränderliche neue Version. Löschen ist technisch gesperrt.")+`<section class="panel"><p><strong>Versionsverwaltung aktiv.</strong> Bearbeiten, Umzugsworkflow und Stilllegen erhalten die vollständige Auditspur.</p>${d.canManage?'<button type="button" id="region-create">Neue Zone anlegen</button>':""}</section><section class="region-zone-grid">${d.groups.map(g=>{const x=g.current;return `<article class="panel region-zone-card"><h2>${esc(x.name)}</h2><p><strong>${esc(x.code)}</strong> · Version ${esc(x.version)} · ${esc(status[x.lifecycle_status]||x.lifecycle_status)}</p><p>${esc((x.regions?.areas||[]).join(", "))}</p>${d.canManage&&x.lifecycle_status==="ACTIVE"?`<div class="review-actions"><button data-region-version="${esc(x.id)}">Neue Version</button><button data-region-relocate="${esc(x.id)}">Umzugsworkflow</button><button data-region-retire="${esc(x.id)}">Stilllegen</button></div>`:""}<details><summary>Versionshistorie (${esc(g.history.length)})</summary><table><tbody>${history([x,...g.history])}</tbody></table></details></article>`}).join("")}</section>`;
      const byId=Object.fromEntries(d.items.map(x=>[x.id,x])),today=new Date().toISOString().slice(0,10),saveVersion=async(zone,changeType)=>{const name=prompt("Name",zone?.name||"");if(!name)return;const reason=prompt("Änderungsbegründung");if(!reason)return;const effectiveFrom=prompt("Wirksam ab (JJJJ-MM-TT)",today);if(!effectiveFrom)return;const body={code:zone?.code||prompt("Zonencode"),name,regions:zone?.regions||{areas:[prompt("Gebiet")].filter(Boolean)},travelRules:zone?.travel_rules||{},effectiveFrom,reason,changeType,expectedContentSha256:zone?.content_sha256};if(changeType==="RELOCATION")body.relocation={from:prompt("Ausgangsort"),to:prompt("Zielort"),effectiveAt:effectiveFrom};await mutate(zone?`/regions/${zone.id}/versions`:"/regions","POST",body);await render()};
      out.querySelector("#region-create")?.addEventListener("click",()=>saveVersion(null,"CREATE"));for(const button of out.querySelectorAll("[data-region-version]"))button.addEventListener("click",()=>saveVersion(byId[button.dataset.regionVersion],"UPDATE"));for(const button of out.querySelectorAll("[data-region-relocate]"))button.addEventListener("click",()=>saveVersion(byId[button.dataset.regionRelocate],"RELOCATION"));for(const button of out.querySelectorAll("[data-region-retire]"))button.addEventListener("click",async()=>{const zone=byId[button.dataset.regionRetire],reason=prompt("Begründung für die Stilllegung"),effectiveFrom=prompt("Wirksam ab (JJJJ-MM-TT)",today);if(!reason||!effectiveFrom)return;await mutate(`/regions/${zone.id}/retire`,"POST",{expectedContentSha256:zone.content_sha256,reason,effectiveFrom});await render()});return;
    }
    const map = {
        sources: "/sources",
  ...
  async function contextView(s) {
    if (!s.tender) return contextRequired(s);
    if(s.view==="participation"){
      const d=await get(`/tenders/${encodeURIComponent(s.tender)}/participation-readiness?company=${encodeURIComponent(s.company)}&lot=${encodeURIComponent(s.lot)}`),actionView={1:"detail",2:"detail",3:"detail",4:"portal-access",5:"portal-access",6:"documents",7:"documents",8:"requirements",9:"company-profiles",10:"calculation",11:"management-output",12:"management-output",13:"offer-documents"};
      const steps=d.steps.map(item=>`<article class="presentation-card participation-step" data-step="${esc(item.number)}"><p class="calc-eyebrow">Schritt ${esc(item.number)} von ${esc(d.steps.length)}</p><h3>${esc(item.title)}</h3>${calcFacts([["Status",calcStatus(item.status)],["Ergebnis",item.result],["Nächster Schritt",item.next],["Zeitpunkt",d.generatedAt],["Gesellschaft",d.companyId],["Verwendete Versionen",item.versions]])}${actionView[item.number]?`<a class="button-link ${item.status==="BLOCKED"?"primary-action":""}" href="${esc(href(actionView[item.number],s))}">${esc(item.next)}</a>`:`<p class="muted">Externe Abgabe bleibt HTTP 423 gesperrt.</p>`}</article>`).join("");
      out.innerHTML=`<nav aria-label="Breadcrumb"><a href="${esc(href("overview",s))}">Tender-Übersicht</a> → <a href="${esc(href("detail",s))}">Tender-Detail</a> → <span>Teilnahme vorbereiten</span></nav>`+title("Teilnahme vorbereiten",`${d.publicationPlatform||"Quelle"} · Gesellschaft und Los sind verbindlich gebunden`)+`<section class="panel"><p><strong>${d.eligible?"Teilnahmefähiges Wettbewerbsverfahren":"Teilnahme derzeit nicht zulässig"}</strong></p><dl><dt>Veröffentlichungsplattform</dt><dd>${esc(d.publicationPlatform||"Nicht ermittelt")}</dd><dt>Teilnahme-/Abgabeportal</dt><dd>${esc(d.participationPortal?.name||"Noch nicht eindeutig ermittelt")}</dd><dt>Portalstatus</dt><dd>${esc(humanizedEnum(d.portalStatus))}</dd><dt>Externe Abgabe</dt><dd>Gesperrt (HTTP 423)</dd></dl></section><section class="participation-grid">${steps}</section>`;
      return;
    }
    ...
    if (s.view === "detail") {
      ...
      const portalItems = (Array.isArray(portals?.items) ? portals.items : []).filter(recordOrNull),
        invalidPortalItems = (Array.isArray(portals?.items) ? portals.items : []).length - portalItems.length,
        eligibilityRecord = recordOrNull(eligibility) || {};
      out.insertAdjacentHTML(
        "beforeend",
        `<section class="panel portal-eligibility"><h3>Portalzugang für die geplante Bietergesellschaft</h3><dl><dt>Portal</dt><dd>${esc(eligibilityRecord.portal_name || "Noch nicht zugeordnet")}</dd><dt>Geplante Bietergesellschaft</dt><dd>${esc(eligibilityRecord.company_name || d.company.legal_name)}</dd><dt>Portalaccount</dt><dd>${esc(eligibilityRecord.account_holder_name || "Nicht autoritativ bestätigt")}</dd><dt>Submissionstatus</dt><dd><strong>${esc(eligibilityRecord.status_label || "Kontextprüfung erforderlich")}</strong></dd><dt>Empfehlung</dt><dd>${esc(eligibilityRecord.recommendation || "Portalidentität rechtzeitig vor der Angebotsabgabe prüfen.")}</dd></dl><p class="muted">Analyse, Dokumentdownload, Kalkulation und Management bleiben unabhängig von diesem Submissionstatus möglich.</p></section>`,
      );
      if (portalItems.length) {
        out.insertAdjacentHTML(
          "beforeend",
          `<section class="panel"><h3>Detailunterlagen und Portalzugang</h3>${invalidPortalItems ? `<p class="error">${esc(invalidPortalItems)} ungültige Portalzugangsdatensätze wurden nicht als fehlender Zugang interpretiert.</p>` : ""}${portalItems
            .map((p) => {
              const approval =
              ...
              const manageUrl = `${BASE}/portalzugaenge/bearbeiten?portalId=${encodeURIComponent(p.portal_id)}&companyId=${encodeURIComponent(s.company)}&tenderId=${encodeURIComponent(s.tender)}&returnTo=${encodeURIComponent(href("detail", s))}`,
                action = p.login_action,
                actionType = p.reconnect_available === true && !["START_LOGIN", "CONFIRM_MFA"].includes(action?.type) ? "START_LOGIN" : action?.type || "NONE",
                actionLabel = p.reconnect_available === true ? p.reconnect_label || "Erneut anmelden" : action?.label || "",
                binding = action?.binding,
                primaryAction = actionType === "MANAGE_CREDENTIALS" && binding ? `<a class="button-link primary-action" data-login-manage="${esc(binding.portal_id)}" href="${esc(manageUrl)}">${esc(actionLabel)}</a>` : actionType === "OPEN_PORTAL_READ_ONLY" && binding && p.portal_open_url ? `<a class="button-link primary-action" data-portal-open="${esc(binding.portal_id)}" href="${esc(p.portal_open_url)}" target="_blank" rel="noopener noreferrer">${esc(actionLabel)}</a>` : actionType === "AUTHENTICATION_TARGET_UNAVAILABLE" && binding ? `<a class="button-link primary-action" data-login-configure="${esc(binding.portal_id)}" href="${esc(manageUrl)}">Portalzugang konfigurieren</a>` : ["START_LOGIN", "CONFIRM_MFA"].includes(actionType) && binding ? `<button type="button" class="primary-action" data-login-portal="${esc(binding.portal_id)}" data-tender="${esc(binding.tender_id)}" data-company="${esc(binding.company_id)}" data-lot="${esc(binding.lot_key)}">${esc(actionLabel)}</button>` : "",
              ...
              const manageLabel = p.credential_status === "NOT_CONFIGURED" ? "Zugangsdaten hinterlegen" : ["EXPIRED", "INVALID"].includes(p.credential_status) ? "Zugang aktualisieren" : "Zugangsdaten verwalten",
                retryLabel = p.documents_complete ? "Kein erneuter Abruf erforderlich" : p.next_retry ? calcDate(p.next_retry) : p.automatic_processing ? "Läuft bereits" : "Nicht eingeplant";
              return `<article data-portal-card="${esc(p.portal_id)}"><dl><dt>Bekanntmachungsquelle</dt><dd>${esc(p.notice_source || d.tender.source_code)}</dd><dt>Zielportal</dt><dd><strong>${esc(p.portal_name)}</strong> · ${esc(p.domain)}</dd><dt>Zugangsstatus</dt><dd><strong>${esc(p.credential_status_label || p.credential_status)}</strong></dd><dt>Dokumentstatus</dt><dd>${esc(p.document_status || p.access_status)}</dd><dt>Globale Freigabe</dt><dd>${esc(p.global_policy_label || "Nicht aktiv")}</dd><dt>Kostenprüfung</dt><dd>${esc(p.cost_check_status || "AUSSTEHEND")}</dd><dt>Kostenklasse</dt><dd><strong>${esc(p.cost_class || "Nicht klassifiziert")}</strong></dd><dt>Nachgewiesener Betrag</dt><dd>${esc(p.proven_amount ?? "–")} ${esc(p.cost_currency || "")}</dd><dt>Nachweisquelle</dt><dd>${esc((p.evidence_source || []).join(", ") || "–")}</dd><dt>Dokumentenanforderung</dt><dd>${esc(p.document_request_status || "–")}</dd><dt>Bieter-/Interessentenliste</dt><dd>${esc([p.bidder_list_status, p.interest_list_status].filter(Boolean).join(" / ") || "–")}</dd><dt>Portalnachrichten</dt><dd>${esc(p.message_status || "–")}</dd><dt>Ursache</dt><dd>${esc(p.login_required_reason)}</dd><dt>Letzter Abruf</dt><dd>${esc(calcDate(p.last_attempt))}</dd><dt>Nächster automatischer Versuch</dt><dd>${esc(retryLabel)}</dd><dt>Aktueller Verarbeitungsschritt</dt><dd>${esc(p.current_processing_step || "Kein aktiver Job")}</dd><dt>Letzter Fehler</dt><dd>${esc(p.last_error || "–")}</dd></dl><h4>Betroffene Dokumente</h4>${portalDocumentList(p.affected_document_items)}<p>Fehlende Kalkulationswerte: ${esc((p.missing_calculation_inputs || []).map((x) => x.field || x).join(", ") || "keine")}</p><div class="review-actions">${approval ? `<button type="button" disabled aria-disabled="true">Freigabe für Dokumentenanforderung erforderlich</button>` : primaryAction || (p.automatic_processing ? `<button type="button" disabled>Automatische Verarbeitung läuft</button>` : "")}${refreshAction}<a class="button-link" href="${esc(manageUrl)}">${esc(manageLabel)}</a>${p.notice_url ? `<a href="${esc(p.notice_url)}" target="_blank" rel="noopener noreferrer">Öffentliche Bekanntmachung öffnen</a>` : ""}</div><p class="muted" data-login-status="${esc(p.portal_id)}" aria-live="polite">${esc(approval ? "Keine externe Aktion ohne ausdrückliche tenderbezogene Freigabe." : p.credential_status_message || "Zugangsstatus noch nicht geprüft.")}</p></article>`;
            })
            .join("")}</section>`,
        );
      } else {
        const portalSearchUrl = `${BASE}/portalzugaenge?companyId=${encodeURIComponent(s.company)}&tenderId=${encodeURIComponent(s.tender)}&mode=search&returnTo=${encodeURIComponent(href("detail", s))}`;
        out.insertAdjacentHTML(
          "beforeend",
          `<section class="panel"><h3>Detailunterlagen und Portalzugang</h3>${invalidPortalItems ? `<p class="error">Portalzugangsstatus enthält ${esc(invalidPortalItems)} ungültige Datensätze. Dies wird nicht als fehlender Zugang behandelt.</p>` : `<p>${esc(portals?.reason || "Vergabeportal in der Quelle nicht angegeben")}</p>`}<a class="button-link" href="${esc(portalSearchUrl)}">Portalzuordnung prüfen</a></section>`,
        );
      }
  ...
  async function load() {
    ...
    try {
      ...
      else if(s.view==="completed-procedures"){
        const d=await get(`/completed-procedures?page=${encodeURIComponent(s.page||1)}&pageSize=50&${s.query?`q=${encodeURIComponent(s.query)}`:""}`),cards=d.items.map(item=>{const original=item.original_tender_id?href("detail",{...s,tender:item.original_tender_id,company:item.company_id,lot:""}):null,source=/^https:\/\//i.test(String(item.source_url||""))?item.source_url:null;return `<article class="presentation-card"><p class="calc-eyebrow">${esc(humanizedEnum(item.notice_classification))}</p><h3>${esc(item.title)}</h3>${calcFacts([["Auftraggeber",item.buyer],["Veröffentlichung",calcDate(item.publication_date)],["Status","Verfahren abgeschlossen"],["Gesellschaft",item.company_name],["Teilnahme","Nicht möglich"]])}<div class="review-actions">${source?`<a class="button-link" href="${esc(source)}" target="_blank" rel="noopener noreferrer">Ergebnis ansehen</a>`:""}${original?`<a class="button-link primary-action" href="${esc(original)}">Ursprüngliche Bekanntmachung öffnen</a>`:""}<a class="button-link" href="${esc(href("detail",{...s,tender:item.id,company:item.company_id,lot:item.lot_key||""}))}">Zugehörige Bekanntmachungen und Historie anzeigen</a></div></article>`}).join("");
        out.innerHTML=title("Abgeschlossene Verfahren","Ergebnisse, Aufhebungen und Auftragsänderungen bleiben historisch erhalten und lösen keine Teilnahmeautomation aus.")+`<section class="panel"><p>${esc(d.total)} abgeschlossene Verfahren</p><div class="presentation-grid">${cards||"<p>Keine abgeschlossenen Verfahren im sichtbaren Gesellschaftsscope.</p>"}</div></section>`;
      }
  ...
  document.addEventListener(
    ...
    async (event) => {
      ...
      const tenderId = button.dataset.tender,
        ...
        correlationKey = [button.dataset.loginPortal,tenderId,companyId,lotKey||""].join(":");
      try {
        ...
        if (!entry) {
          ...
          const continuation = await post(
            ...
            { tender_id: tenderId, company_id: companyId, lot_key: lotKey, blocked_action: button.dataset.blockedAction || "DOCUMENT_FETCH" },
          );
        ...
        const poll = async () => {
          const state = await post(
            ...
            { correlationToken: continuation.correlationToken },
          );
          ...
          if (processingMessages[state.status]) {
            ...
            if (["AUTOMATIC_PROCESSING_PLANNED","AUTOMATIC_PROCESSING_ACTIVE","DOCUMENT_DOWNLOAD_ACTIVE"].includes(state.status)) setTimeout(()=>poll().catch((error)=>fail(null,error)),2000); else { entry.polling=false; button.disabled=false; button.removeAttribute("aria-busy"); }
            if (state.sessionValid) { correlatedPortalLogins.delete(correlationKey); button.textContent="Portal verbunden"; button.disabled=true; button.removeAttribute("aria-busy"); if(status)status.textContent=`Portal verbunden · geprüft ${new Intl.DateTimeFormat("de-DE",{dateStyle:"medium",timeStyle:"short"}).format(new Date())}. Der blockierte Dokumentabruf wird fortgesetzt.`; setTimeout(()=>location.reload(),1200); }
            return;
          }
          ...
          setTimeout(() => poll().catch((error) => fail(null,error)), 2000);
        };
        await poll();
      } catch (error) {
        correlatedPortalLogins.delete(correlationKey);
