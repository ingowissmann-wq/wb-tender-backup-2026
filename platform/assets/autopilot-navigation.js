...
<details class="panel calc-technical"><summary>Revisionsnachweise</summary>${calcFacts(
  ...
  async function submissionStatusView(s) {
    ...
    document.querySelector("#approve-live-submission")?.addEventListener("click",async()=>{try{await mutate(`/submission/v2/${encodeURIComponent(liveSubmission.item.id)}/approve`,"POST",{});liveStatus.textContent="Vier-Augen-Freigabe mit aktueller WB-MFA gespeichert.";await submissionStatusView(s)}catch(error){liveStatus.textContent=error.message}});
    document.querySelector("#confirm-live-submission")?.addEventListener("click",async()=>{const confirmation=prompt(`Bitte bestätigen Sie den rechtsverbindlichen Versand durch vollständige Eingabe:\n\n${legalText}`)||"";if(confirmation!==legalText){liveStatus.textContent="Versand nicht bestätigt; es wurde nichts übermittelt.";return}try{await mutate(`/submission/v2/${encodeURIComponent(liveSubmission.item.id)}/confirm`,"POST",{confirmation});liveStatus.textContent="Versand bestätigt und für die technischen Prüfphasen vorgemerkt.";await submissionStatusView(s)}catch(error){liveStatus.textContent=error.message}});
  }
  async function load() {
    const s = state();
    navigation();
