...
async function boundedFetch(fetchImpl,url,options,portal,jar=new Map()){
  const allowed=[...(portal.allowed_subdomains||[]),...(portal.authentication_domains||[]),...(portal.download_domains||[])];
  const credentialTargets=[...(portal.allowed_subdomains||[]),...(portal.authentication_domains||[])];
  let request={...options,headers:{...(options.headers||{})}};
  // Validate the initial target as well as redirects. HTML form actions are untrusted.
  let current=canonicalPortalUrl(String(url),portal.canonical_domain,request.body?credentialTargets:allowed);
  const maximum=Math.min(12,Math.max(1,Number(portal.max_redirects||8)));
  for(let redirect=0;redirect<maximum;redirect++){
    if(fetchImpl===fetch)await assertPublicDns(current.hostname);
    const headers={...request.headers},cookies=cookieHeader(jar,current);if(cookies)headers.cookie=cookies;
    const response=await fetchImpl(current,{...request,headers,redirect:"manual",signal:AbortSignal.timeout(Number(portal.timeout_profile?.navigationMs||15_000))});absorbCookies(response,jar,current);
    if([301,302,303,307,308].includes(response.status)&&response.headers.get("location")){
      if(redirect===maximum-1)throw new Error("login_redirect_unexpected");
      let next;try{next=canonicalPortalUrl(new URL(response.headers.get("location"),current).href,portal.canonical_domain,allowed)}catch{throw new Error("login_redirect_unexpected")}
      const method=String(request.method||'GET').toUpperCase();
      if(response.status===303&&method!=='HEAD'||[301,302].includes(response.status)&&method==='POST'){
        request={...request,method:'GET'};delete request.body;
        for(const key of Object.keys(request.headers))if(['content-type','content-length','origin'].includes(key.toLowerCase()))delete request.headers[key];
      }else if(request.body&&next.origin!==current.origin){
        // A 307/308 must not replay credentials to another origin, including a download host.
        throw new Error('login_redirect_unexpected');
      }
      if(next.origin!==current.origin)for(const key of Object.keys(request.headers))if(['authorization','cookie'].includes(key.toLowerCase()))delete request.headers[key];
