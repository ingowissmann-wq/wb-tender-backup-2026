...
function normalizedEmail(value){const email=String(value||'').trim().toLowerCase();if(!EMAIL.test(email)||email.length>254)fail('email_invalid');return email;}

async function assertTrialEligible(client,email,identityHash,{ignoreIntentId=null}={}){
  ...
  if((await client.query('SELECT 1 FROM saas.tenants WHERE customer_identity_hash=$1 LIMIT 1',[identityHash])).rowCount)fail('trial_not_eligible',409);
  ...
  if((await client.query("SELECT 1 FROM saas.pending_registrations WHERE lower(email)=$1 AND status NOT IN('CANCELLED','EXPIRED') LIMIT 1",[email])).rowCount)fail('trial_not_eligible',409);
  const active=await client.query(`SELECT id FROM saas.trial_onboarding_intents
    WHERE customer_identity_hash=$1 AND status IN('EMAIL_VERIFICATION_PENDING','EMAIL_VERIFIED','CHECKOUT_CREATED','ACCOUNT_SETUP_PENDING','MFA_SETUP_PENDING')
      AND ($2::uuid IS NULL OR id<>$2::uuid) LIMIT 1`,[identityHash,ignoreIntentId]);
  if(active.rowCount)fail('trial_onboarding_already_started',409);
}

export async function beginEmailFirstTrial(pool,{email,requestIp='',userAgent='',verificationPepper,now=new Date()}){
  email=normalizedEmail(email);
  const identityHash=customerIdentityHash(email,verificationPepper), token=verificationToken(), tokenHash=hashVerificationToken(token,verificationPepper);
  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    await assertTrialEligible(client,email,identityHash);
    const row=(await client.query(`INSERT INTO saas.trial_onboarding_intents
      ...
      VALUES($1,$2,$3,$4,$5,$6) RETURNING id,email,booking_id`,[email,identityHash,tokenHash,new Date(now.getTime()+24*60*60*1000),digest(requestIp),digest(userAgent)])).rows[0];
    await client.query('COMMIT');
    return {...row,token};
  }catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}
}

export async function verifyEmailAndCreateCheckout(pool,{token,verificationPepper,fieldEncryptionKey,billingAdapter,now=new Date()}){
  const tokenHash=hashVerificationToken(String(token||''),verificationPepper), client=await pool.connect();
  try{
    await client.query('BEGIN');
    const intent=(await client.query(`SELECT * FROM saas.trial_onboarding_intents WHERE verification_token_hash=$1 FOR UPDATE`,[tokenHash])).rows[0];
    ...
    await assertTrialEligible(client,intent.email,intent.customer_identity_hash,{ignoreIntentId:intent.id});
    ...
    const placeholderPassword=await hashTenderPassword(crypto.randomBytes(32).toString('base64url'));
    const placeholderMfa=encryptTotpSecret(randomTotpSecret(),fieldEncryptionKey);
    ...
    await client.query(`INSERT INTO saas.pending_registrations
      (tenant_id,email,requested_plan_code,verification_token_hash,verification_expires_at,email_verified_at,status,request_ip_hash,request_user_agent_hash,password_hash,mfa_secret_encrypted,billing_path,purchase_kind,booking_id,consent_version,consented_at)
      VALUES($1,$2,'ENTERPRISE',NULL,NULL,$3,'PAYMENT_PENDING',$4,$5,$6,$7,'AUTO_CARD','TRIAL',$8,$9,$3)`,
      [tenantId,intent.email,now,intent.request_ip_hash,intent.request_user_agent_hash,placeholderPassword,placeholderMfa,intent.booking_id,BOOKING_TERMS_VERSION]);
    await client.query("INSERT INTO saas.subscriptions(tenant_id,plan_code,status) VALUES($1,'ENTERPRISE','PENDING_PAYMENT')",[tenantId]);
    await client.query(`UPDATE saas.trial_onboarding_intents SET tenant_id=$2,email_verified_at=$3,status='EMAIL_VERIFIED',verification_token_hash=NULL,updated_at=$3 WHERE id=$1`,[intent.id,tenantId,now]);
    await client.query('COMMIT');

    const contract=bookingContract({purchaseKind:'TRIAL',plan:'TRIAL',billingPath:'AUTO_CARD',consentVersion:BOOKING_TERMS_VERSION,bookingConfirmed:true});
    const checkout=await createBoundCheckout(pool,billingAdapter,tenantId,contract,intent.booking_id);
    await pool.query(`UPDATE saas.trial_onboarding_intents SET checkout_provider=$2,checkout_ref=$3,status='CHECKOUT_CREATED',updated_at=now() WHERE id=$1 AND tenant_id=$4`,[intent.id,billingAdapter.provider,checkout.id,tenantId]);
    return {checkoutUrl:checkout.url,tenantId};
  }catch(error){if(client.query)await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}
}

export async function markEmailFirstPaymentPendingSetup(client,tenantId,now=new Date()){
  const result=await client.query(`UPDATE saas.trial_onboarding_intents SET status='ACCOUNT_SETUP_PENDING',payment_confirmed_at=coalesce(payment_confirmed_at,$2),updated_at=$2
    WHERE tenant_id=$1 AND status IN('CHECKOUT_CREATED','ACCOUNT_SETUP_PENDING') RETURNING id`,[tenantId,now]);
  if(!result.rowCount)return false;
  await client.query("UPDATE saas.pending_registrations SET status='ACCOUNT_SETUP_PENDING',updated_at=$2 WHERE tenant_id=$1 AND iam_provisioned_at IS NULL",[tenantId,now]);
  return true;
}
...
export async function beginPaidAccountSetup(pool,{sessionId,company,password,passwordConfirmation,verificationPepper,fieldEncryptionKey,now=new Date()}){
  ...
  company=String(company||'').trim().slice(0,160);if(company.length<2)fail('company_name_invalid');
  password=String(password||'');if(password.length<12||password.length>128||password!==String(passwordConfirmation||''))fail('password_invalid');
  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const row=(await client.query(`SELECT i.*,c.status checkout_status,p.iam_provisioned_at
      FROM saas.trial_onboarding_intents i JOIN saas.checkout_sessions c ON c.tenant_id=i.tenant_id AND c.provider_checkout_ref=i.checkout_ref
    ...
    const mfaSecret=randomTotpSecret(), setupToken=verificationToken(), setupHash=hashVerificationToken(setupToken,verificationPepper);
    await client.query(`UPDATE saas.pending_registrations SET password_hash=$2,mfa_secret_encrypted=$3,status='ACCOUNT_SETUP_PENDING',updated_at=$4 WHERE tenant_id=$1`,[row.tenant_id,await hashTenderPassword(password),encryptTotpSecret(mfaSecret,fieldEncryptionKey),now]);
    await client.query('UPDATE saas.tenants SET display_name=$2,updated_at=$3 WHERE id=$1',[row.tenant_id,company,now]);
    await client.query(`UPDATE saas.trial_onboarding_intents SET status='MFA_SETUP_PENDING',setup_token_hash=$2,setup_expires_at=$3,updated_at=$4 WHERE id=$1`,[row.id,setupHash,new Date(now.getTime()+15*60*1000),now]);
    await client.query('COMMIT');
    ...
    return {setupToken,mfaManualKey:mfaSecret,mfaQrCode:await QRCode.toDataURL(uri,{errorCorrectionLevel:'M',margin:2,width:220})};
  }catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}
}

export async function finishPaidAccountSetup(pool,{setupToken,mfaCode,verificationPepper,fieldEncryptionKey,now=new Date()}){
  const setupHash=hashVerificationToken(String(setupToken||''),verificationPepper),client=await pool.connect();
  try{
    await client.query('BEGIN');
    const row=(await client.query(`SELECT i.*,p.mfa_secret_encrypted,p.iam_provisioned_at,t.display_name
      FROM saas.trial_onboarding_intents i JOIN saas.pending_registrations p ON p.tenant_id=i.tenant_id JOIN saas.tenants t ON t.id=i.tenant_id
    ...
    await client.query("UPDATE saas.pending_registrations SET status='ACTIVATED',verification_token_hash=NULL,updated_at=$2 WHERE tenant_id=$1",[row.tenant_id,now]);
    await client.query("UPDATE saas.trial_onboarding_intents SET status='ACTIVATED',activated_at=$2,setup_token_hash=NULL,setup_expires_at=NULL,updated_at=$2 WHERE id=$1",[row.id,now]);
    await client.query("INSERT INTO saas.audit_events(tenant_id,actor_user_id,action,target_type,target_id,metadata) VALUES($1,$2,'EMAIL_FIRST_TRIAL_ACTIVATED','tenant',$1::uuid::text,$3)",[row.tenant_id,userId,{productionAccessGranted:true,mfaVerified:true}]);
    await client.query('COMMIT');
...
export function registerEmailFirstTrialRoutes(app,{pool,guard,verificationPepper,fieldEncryptionKey,emailAdapter,billingAdapter}){
  ...
  app.post('/api/saas/trial/email',{preHandler:guard,config:{rateLimit:{max:8,timeWindow:'1 hour'}}},async(req,reply)=>{try{const created=await beginEmailFirstTrial(pool,{email:req.body?.email,requestIp:req.ip,userAgent:req.headers['user-agent'],verificationPepper});if(emailAdapter?.configured)await emailAdapter.sendVerification({email:created.email,token:created.token,tenantId:null,verificationPath:'/saas/trial/verify'});return reply.code(202).send({status:'EMAIL_VERIFICATION_PENDING'});}catch(error){return reply.code(error.statusCode||400).send({error:error.message});}});
