...
async function processDocuments(pool, enrichment, selected = []) {
  ...
  for (const document of documents) {
    ...
    const source = new URL(accessUrl),persistedPublicEvidence=(await pool.query(`SELECT EXISTS(SELECT 1 FROM tender.tender_external_links link WHERE link.tender_id=$1 AND link.public_access=true AND link.role IN('NOTICE','NOTICE_VIEW','PUBLIC_DOCUMENT','PROCUREMENT_DOCUMENT') AND ($2=link.original_url OR $2=link.final_url)) ok`,[enrichment.tender_id,accessUrl])).rows[0]?.ok===true,
      publicRib =
      ...
      authHeaders=persistedPublicEvidence||publicRib || publicEvergabeOnline || publicNetServerPortal || publicAIBietercockpitPortal || publicDuesseldorfNetServer
          ? {}
    ...
    workforceCapacityRow = (
      await pool.query(
        `SELECT
         ...
         ORDER BY version.version_no DESC,active.activated_at DESC
         LIMIT 1`,
        [item.company_id, item.service_scope],
      )
    ).rows[0],
...
async function processAction(pool, item, tender, run) {
  ...
  if (
    ...
    const persistedPublicDocumentScope=(await pool.query(`SELECT EXISTS(SELECT 1 FROM tender.tender_external_links link
      WHERE link.tender_id=$1 AND link.public_access=true
        AND link.role IN('NOTICE','NOTICE_VIEW','PUBLIC_DOCUMENT','PROCUREMENT_DOCUMENT')) ok`,[tender.id])).rows[0]?.ok===true;
    if(!persistedPublicDocumentScope)await materializeAuthoritativePortalAssignments(pool, {
      tenderId: tender.id,
      selected,
    });
    const exactDocumentAssignments = persistedPublicDocumentScope ? [{public_document:true}] : (await pool.query(
      `SELECT 1 FROM tender.current_tender_company_portal_role_scopes scope
       WHERE scope.tender_id=$1 AND scope.company_id=$2 AND scope.source_lot_id=$3
         AND scope.portal_id=$4 AND scope.portal_role='DOCUMENT_PORTAL'
         AND ($5::text IS NULL OR scope.canonical_service=$5) LIMIT 2`,
    ...
    const resolution =
      ...
      portalAccess = verifiedSession
        ? "LOGIN_SUCCEEDED"
        : persistedPublicDocumentScope && !item.portal_id
...
export async function processQueueItem(pool, item) {
  ...
  try {
    ...
    const terminalCodes = new Set([
      ...
      interactiveLoginAttempt=item.action_type==="START_PORTAL_AUTHENTICATION",
      retry=!interactiveLoginAttempt&&!terminalCodes.has(code) &&
...
export async function claimQueue(pool) {
  ...
  return (
    (
      await pool.query(
        `UPDATE tender.autopilot_queue SET status='RUNNING',current_step='CLAIMED',started_at=now(),heartbeat_at=now(),last_progress_at=now(),timeout_at=CASE WHEN action_type IN('START_PORTAL_AUTHENTICATION','TEST_PORTAL_CONNECTION','TEST_DOCUMENT_FETCH') THEN now()+interval '3 minutes' ELSE timeout_at END,attempt=attempt+1,claimed_at=now(),worker_id=$1 WHERE id=(SELECT id FROM tender.autopilot_queue WHERE status IN ('PENDING','RETRY','QUEUED') AND next_attempt_at<=now() ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING *`,
        [
          String(
