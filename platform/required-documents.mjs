...
export const submissionDocumentsComplete = (requirements) =>
  requirements.filter((r)=>r.mandatory && r.submission_relevant)
    .every((r)=>["VALIDATED","NOT_REQUIRED"].includes(r.satisfaction_status));
...
export function requirementLabel(status) {
  return ({MISSING:"Einzureichendes Dokument fehlt",AVAILABLE:"Einzureichendes Dokument vorhanden",UPLOADED_PENDING_VALIDATION:"Dokument wurde hochgeladen und wird geprüft",MANUAL_REVIEW_REQUIRED:"Dokument hochgeladen – fachliche Prüfung erforderlich",VALIDATED:"Dokument geprüft und vollständig",REJECTED:"Dokument passt nicht zur geforderten Unterlage",NOT_REQUIRED:"Nicht erforderlich",SUPERSEDED:"Durch neuere Anforderung ersetzt"})[status] || "Status nicht verfügbar";
}
