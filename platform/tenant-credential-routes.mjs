import {TenantPortalSessions} from './tenant-portal-sessions.mjs';
...
export function registerTenantCredentialRoutes(app,{pool,authenticate,csrf,keyringFile}) {
  ...
  const sessions=new TenantPortalSessions({pool,keyringFile});
  ...
  const safe=fn=>async(req,reply)=>{
    ...
    try { return await fn(req,reply); }
      ...
      const known=/^(portal_credential_|portal_session_|credential_|company_not_found$|portal_not_found$)/.test(code);
  ...
  app.get(path+'/:id/sessions',{preHandler:guards},safe(req=>sessions.list(req.tenant,req.params.companyId,req.params.id)));
  app.post(path+'/:id/check',{preHandler:[...guards,csrf],bodyLimit:1024},safe(req=>sessions.check(req.tenant,req.params.companyId,req.params.id,req.body)));
  app.post(path+'/:id/sessions/:sessionId/verify',{preHandler:[...guards,csrf],bodyLimit:1024},safe(req=>sessions.restoreStored(req.tenant,req.params.companyId,req.params.id,req.params.sessionId)));
...
async function load(){list.replaceChildren();if(!form.elements.companyId.value)return;for(const item of await request('/api/tenant-portal/companies/'+encodeURIComponent(form.elements.companyId.value)+'/credentials')){const row=document.createElement('li');row.textContent=item.label+' · Zugang gespeichert · Version '+item.revision+' ';const edit=document.createElement('button');edit.type='button';edit.textContent='Ändern';edit.addEventListener('click',()=>{form.dataset.id=item.id;form.dataset.revision=item.revision;form.elements.portalId.value=item.portal_id;form.elements.portalId.disabled=true;form.elements.label.value=item.label;form.elements.username.value='';form.elements.password.value='';result.textContent='Aktuelle Zugangsdaten erneut eingeben. Gespeicherte Passwörter werden nicht zurückgesendet.'});row.append(edit);const base='/api/tenant-portal/companies/'+encodeURIComponent(form.elements.companyId.value)+'/credentials/'+item.id;const check=document.createElement('button');check.type='button';check.textContent='Anmeldung prüfen';check.onclick=async()=>{check.disabled=true;result.textContent='Portal-Anmeldung und Sitzungswiederherstellung werden geprüft.';try{check.dataset.requestId||=crypto.randomUUID();const token=decodeURIComponent(document.cookie.split('; ').find(x=>x.startsWith('wb_csrf='))?.slice(8)||'');const saved=await request(base+'/check',{method:'POST',headers:{'Content-Type':'application/json','x-csrf-token':token},body:JSON.stringify({id:check.dataset.requestId,expectedRevision:item.revision})});result.textContent=saved.status==='VERIFIED'?'Anmeldung und gespeicherte Sitzung geprüft.':saved.status==='MFA_REQUIRED'?'Das Vergabeportal verlangt eine zusätzliche Bestätigung.':saved.status==='CHECKING'?'Die Prüfung läuft noch.':'Anmeldung nicht bestätigt: '+saved.result_code;await load()}catch(error){result.textContent=error.message}finally{check.disabled=false}};row.append(check);const sessions=await request(base+'/sessions');for(const session of sessions){const state=document.createElement('span');state.textContent=' · Sitzung: '+(session.usable?'verfügbar':session.status==='VERIFIED'?'abgelaufen oder Zugang geändert':session.status);row.append(state);if(session.usable){const verify=document.createElement('button');verify.type='button';verify.textContent='Gespeicherte Sitzung prüfen';verify.onclick=async()=>{verify.disabled=true;try{const token=decodeURIComponent(document.cookie.split('; ').find(x=>x.startsWith('wb_csrf='))?.slice(8)||'');const checked=await request(base+'/sessions/'+session.id+'/verify',{method:'POST',headers:{'Content-Type':'application/json','x-csrf-token':token},body:'{}'});result.textContent=checked.verified?'Gespeicherte Sitzung erfolgreich wiederhergestellt.':'Gespeicherte Sitzung nicht mehr gültig.';await load()}catch(error){result.textContent=error.message}finally{verify.disabled=false}};row.append(verify)}}list.append(row)}}
