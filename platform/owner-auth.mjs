...
const authCss = `:root{font-family:system-ui,-apple-system,sans-serif;color:#172033;background:#f6f7f9}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:1rem}.panel{width:min(100%,32rem);background:#fff;border:1px solid #d8dee7;border-radius:12px;padding:clamp(1.25rem,4vw,2rem);box-shadow:0 8px 30px rgb(15 23 41/.1)}h1{margin:.2rem 0 1rem;color:#0f1729}p{line-height:1.5}.muted{color:#5d6878}label{display:grid;gap:.35rem;margin:1rem 0;font-weight:650}input{width:100%;min-height:46px;padding:.7rem;border:1px solid #aeb8c6;border-radius:7px;font:inherit}button,.button{display:inline-block;min-height:46px;padding:.75rem 1rem;border:0;border-radius:7px;background:#087173;color:white;font:650 1rem system-ui;cursor:pointer;text-decoration:none}button[disabled]{opacity:.65}.error{color:#9b1c1c;background:#fff1f0;border-left:4px solid #b42318;padding:.75rem}.notice{background:#edf7f7;border-left:4px solid #087173;padding:.75rem}.secret,.codes{font:600 .95rem ui-monospace,monospace;overflow-wrap:anywhere;background:#f2f4f7;padding:.75rem;border-radius:6px}.hidden{display:none}a{color:#086f72}`;

const authJs = `(()=>{const base="/admin/ausschreibungen",api="/api/tender/auth",mode=document.body.dataset.mode,status=document.querySelector("#status"),show=(m,e=false)=>{status.textContent=m;status.className=e?"error":"notice"},post=async(path,body)=>{const r=await fetch(api+path,{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify(body)});let data={};try{data=await r.json()}catch{}if(!r.ok)throw new Error(data.message||data.error||"Anfrage fehlgeschlagen");return data},safeReturn=()=>{const value=new URLSearchParams(location.search).get("returnTo")||base+"/";return value===base||value.startsWith(base+"/")?value:base+"/"};if(mode==="first"){const params=new URLSearchParams(location.search),firstToken=params.get("token")||"";history.replaceState({},"",base+"/first-login");document.querySelector("form").onsubmit=async e=>{e.preventDefault();const b=new FormData(e.target);try{e.submitter.disabled=true;await post("/first-login",{token:firstToken,password:b.get("password"),confirmation:b.get("confirmation")});show("Passwort gespeichert. Melden Sie sich jetzt an und richten Sie die Zwei-Faktor-Authentifizierung ein.");e.target.classList.add("hidden");document.querySelector("#login-link").classList.remove("hidden")}catch(x){show(x.message,true)}finally{if(e.submitter)e.submitter.disabled=false}};return}let challenge=null,target=safeReturn();const login=document.querySelector("#login-form"),mfa=document.querySelector("#mfa-form");login.onsubmit=async e=>{e.preventDefault();const b=new FormData(login);try{e.submitter.disabled=true;const data=await post("/login",{email:b.get("email"),password:b.get("password")});challenge=data.challenge;login.classList.add("hidden");mfa.classList.remove("hidden");if(data.mfaSetupRequired){document.querySelector("#setup").classList.remove("hidden");document.querySelector("#secret").textContent=data.secret;document.querySelector("#uri").textContent=data.uri}show("Passwort bestätigt. Bitte den zweiten Faktor eingeben.")}catch(x){show(x.message,true)}finally{if(e.submitter)e.submitter.disabled=false}};mfa.onsubmit=async e=>{e.preventDefault();const b=new FormData(mfa);try{e.submitter.disabled=true;const data=await post("/mfa",{challenge,code:b.get("code")});if(data.recoveryCodes){mfa.classList.add("hidden");document.querySelector("#recovery").classList.remove("hidden");document.querySelector("#codes").textContent=data.recoveryCodes.join("\\n");document.querySelector("#continue").onclick=()=>location.assign(target);show("Zwei-Faktor-Authentifizierung ist aktiv. Bewahren Sie die Einmal-Wiederherstellungscodes sicher auf.")}else location.assign(target)}catch(x){show(x.message,true)}finally{if(e.submitter)e.submitter.disabled=false}}})();`;

function page(mode, uiBase) {
  const first = mode === "first";
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="referrer" content="no-referrer"><title>${first ? "Erster Zugang" : "Anmeldung"} · WB Tender</title><link rel="stylesheet" href="${uiBase}/auth.css"><script src="${uiBase}/auth.js" defer></script></head><body data-mode="${mode}"><main class="panel"><h1>${first ? "Sicheren Zugang einrichten" : "WB Tender anmelden"}</h1><p class="muted">Geschützter Eigentümerzugang. Externe Angebotsabgaben bleiben technisch gesperrt.</p><p id="status" class="muted" aria-live="polite">${first ? "Legen Sie jetzt Ihr persönliches Passwort fest." : "Melden Sie sich mit Ihrem persönlichen Passwort an."}</p>${first ? `<form><label>Neues Passwort<input name="password" type="password" minlength="14" maxlength="256" autocomplete="new-password" required></label><label>Passwort bestätigen<input name="confirmation" type="password" minlength="14" maxlength="256" autocomplete="new-password" required></label><button type="submit">Passwort sicher speichern</button></form><a id="login-link" class="button hidden" href="${uiBase}/login">Zur Anmeldung</a>` : `<form id="login-form"><label>E-Mail<input name="email" type="email" maxlength="254" autocomplete="username" value="${TARGET_EMAIL}" required></label><label>Passwort<input name="password" type="password" minlength="14" maxlength="256" autocomplete="current-password" required></label><button type="submit">Weiter</button></form><form id="mfa-form" class="hidden"><section id="setup" class="hidden"><p>Richten Sie den Schlüssel in Ihrer Authenticator-App ein. Eine Sitzung entsteht erst nach erfolgreicher TOTP-Prüfung.</p><p id="secret" class="secret"></p><p id="uri" class="secret"></p></section><label>Authenticator- oder Wiederherstellungscode<input name="code" minlength="6" maxlength="64" autocomplete="one-time-code" required></label><button type="submit">Sicher anmelden</button></form><section id="recovery" class="hidden"><h2>Wiederherstellungscodes</h2><p>Jeder Code ist nur einmal verwendbar.</p><pre id="codes" class="codes"></pre><button id="continue" type="button">Zum Tender-System</button></section>`}</main></body></html>`;
}
...
export function registerOwnerAuth(app, { pool, authenticate, csrf, uiBase, apiBase, sessionPepper }) {
  ...
  app.post("/api/auth/login", { preHandler: origin, config: { rateLimit: { max: 12, timeWindow: "15 minutes" } } }, async (req, reply) => {
    ...
    return setupSecret
      ? { mfaRequired: true, mfaSetupRequired: true, challenge: rawChallenge, secret: setupSecret, uri: `otpauth://totp/WB%20Tender:${encodeURIComponent(TARGET_EMAIL)}?secret=${setupSecret}&issuer=WB%20Tender&algorithm=SHA1&digits=6&period=30` }
