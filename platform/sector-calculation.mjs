...
export const CALCULATION_SCHEMA_VERSION = 5;
...
export function calculateSectorTender({
  ...
  const fteAnnualHours = numeric(parameters.C23);
  const fteAnnualHoursUnit = unitFor("C23");

  if (!Number.isFinite(fteAnnualHours) || fteAnnualHours <= 0) {
    missing.push("C23 Produktive Jahresstunden je Vollzeitkraft");
    missingDetails.push({
      field: "C23 Produktive Jahresstunden je Vollzeitkraft",
      parameterKey: "C23",
      unit: "HOURS_PER_YEAR",
      source: "aktive gesellschafts- und leistungsscharfe Kalkulationsparameter",
      nextAction: "C23 im Adminportal fachlich freigeben und aktivieren",
    });
  } else if (fteAnnualHoursUnit !== "HOURS_PER_YEAR") {
    missing.push("C23 Einheit HOURS_PER_YEAR");
    missingDetails.push({
      field: "C23 Einheit",
      parameterKey: "C23",
      unit: "HOURS_PER_YEAR",
      suppliedUnit: fteAnnualHoursUnit,
      source: "aktive gesellschafts- und leistungsscharfe Kalkulationsparameter",
      nextAction: "C23 mit der Einheit Stunden/Jahr freigeben",
    });
  }

  for (const key of ["C04", "C05", "C06", "C07", "C18"]) {
    const value = numeric(parameters[key]);
    if (value === null || (value >= 0 && value <= 100)) continue;
    missing.push(`${key} Prozentwert außerhalb 0–100`);
    missingDetails.push({
      field: `${key} Prozentwert`,
      parameterKey: key,
      unit: "PERCENT",
      suppliedValue: value,
      source: "aktive gesellschafts- und leistungsscharfe Kalkulationsparameter",
      nextAction: `${key} im Adminportal fachlich prüfen und neu freigeben`,
    });
  }
...
export function buildManagementOutput({tender,lotKey,company,profileSnapshot,documentRevision,calculation,missing=[],jobId,correlationId,now=new Date().toISOString()}={}) {
  ...
  const output={schemaVersion:4,status:calculated?"MANAGEMENT_OUTPUT_GENERATED":missingFacilityProfile?"CALCULATION_BLOCKED_MISSING_FACILITY_PROFILE":"NICHT_KALKULIERBAR_FEHLENDE_TENDERUNTERLAGEN",executiveSummary:{buyer:tender?.buyer||null,tender:tender?.title||null,lot:lotKey||null,serviceArea:company?.sector_slug||null,deadline:tender?.offer_deadline||null},recommendation:{decision:partial?"MANAGEMENT_REVIEW_REQUIRED_PARTIAL":calculated?"CONDITIONAL_GO":"NICHT_ANGEBOTSFÄHIG",reason:partial?"Belastbare Teilkalkulation liegt vor; ausgewiesene Einzelpositionen benötigen noch einen quellengebundenen Kostenansatz.":calculated?"Kalkulation liegt vollständig und quellengebunden zur fachlichen Entscheidung vor.":missingFacilityProfile?"Für company_id und Facility-Service-Line existiert kein aktives, freigegebenes Kalkulationsprofil; eine fachfremde Fallback-Auflösung ist gesperrt.":"Tenderbezogene Pflichtangaben fehlen.",requiredActions:missing.map(item=>item.field||item)},calculation:calculated?calculation:null,personnel:calculated?{productiveHours:calculation.productiveHours,fte:calculation.fte}:null,risks:{classification:calculated?"FACHLICHE_PRÜFUNG_ERFORDERLICH":missingFacilityProfile?"FACILITY_PROFILE_MISSING":"DOKUMENTENRISIKO",items:[]},capacity:{status:calculated?"AUS_EFFECTIVE_PROFILE_GEBUNDEN":missingFacilityProfile?"FACILITY_PROFILE_REQUIRED":"NACH_DOKUMENTEINGANG_NEU_BEWERTEN"},awardChance:{value:null,confidence:"NOT_ENOUGH_AUTHORITATIVE_DATA",invented:false},evidence:{missing,profileComplete:!missingFacilityProfile},nextSteps:calculated?[{action:"BOARD_REVIEW",priority:"HIGH"}]:missingFacilityProfile?[{action:"APPROVE_FACILITY_CALCULATION_PROFILE",priority:"HIGH"}]:[{action:"AUTOMATIC_DOCUMENT_REFETCH",priority:"HIGH"}],provenance:{profileSnapshotId:profileSnapshot?.id||null,profileRevision:profileSnapshot?.revision||null,documentRevision,calculationVersion:calculation?.schemaVersion||null,managementOutputVersion:4,jobId,correlationId,generatedAt:now},externalTransmission:false};
  const deterministicOutput={...output,provenance:{...output.provenance,jobId:null,correlationId:null,generatedAt:null}};
  const outputHash=snapshotHash(deterministicOutput);
  return {...output,outputHash,receiptHash:snapshotHash({...output,outputHash})};
}
