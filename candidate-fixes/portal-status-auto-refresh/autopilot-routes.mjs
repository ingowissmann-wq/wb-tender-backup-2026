          ...
          `SELECT e.*,t.id tender_id,scope.company_id,
           ...
           LEFT JOIN LATERAL(SELECT region.* FROM tender.region_evaluations region
             WHERE region.tender_id=t.id AND region.company_id=scope.company_id
               AND (region.lot_id IS NOT DISTINCT FROM canonical_lot.id OR region.lot_id IS NULL)
             ...
             ORDER BY
               (region.lot_id IS NOT DISTINCT FROM canonical_lot.id) DESC,
               region.evaluation_version DESC,
               region.created_at DESC,
               region.id DESC
             LIMIT 1)e ON true
      ...
      const lots = (
        await pool.query(
          `SELECT e.*,l.id,life.lot_key external_id,coalesce(l.title,enriched.title,life.lot_key) title,
           ...
           LEFT JOIN LATERAL(SELECT region.* FROM tender.region_evaluations region
             WHERE region.tender_id=life.tender_id AND region.company_id=$2
               AND (region.lot_id=l.id OR region.lot_id IS NULL)
