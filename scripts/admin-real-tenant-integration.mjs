...
try {
  ...
  const internalBaseline = {
    resources: await scalar(internal, "SELECT count(*) FROM app.resources"),
    files: await scalar(internal, "SELECT count(*) FROM files.objects"),
  };
  ...
  if (await scalar(internal, "SELECT count(*) FROM app.resources") !== internalBaseline.resources) throw new Error("internal_resources_changed");
  if (await scalar(internal, "SELECT count(*) FROM files.objects") !== internalBaseline.files) throw new Error("internal_files_changed");
