...
const uiBase = process.env.E2E_UI_BASE || "/admin/ausschreibungen";
const apiBase = process.env.E2E_API_BASE || "/api";
if (!/^\/[A-Za-z0-9_~.-]+(?:\/[A-Za-z0-9_~.-]+)*$/.test(uiBase) || !/^\/[A-Za-z0-9_~.-]+(?:\/[A-Za-z0-9_~.-]+)*$/.test(apiBase)) throw new Error("e2e_base_path_invalid");
...
const login = async (page, returnTo, code) => {
  await page.goto(`${baseURL}${uiBase}/login?returnTo=${encodeURIComponent(returnTo)}`);
  ...
  const mfaResponse = page.waitForResponse((response) => response.request().method() === "POST" && response.url().endsWith(`${apiBase}/iam/mfa`));
...
try {
  ...
  page.on("request", (request) => {
    ...
    if (pathname.includes("/iam/")) authRequests.push(pathname);
  });
  ...
  const canonical = await request("get", uiBase, { maxRedirects: 0 });
  assert.equal(canonical.status(), 308);
  assert.equal(canonical.headers().location, `${uiBase}/`);
  ...
  await page.waitForURL(`${baseURL}${uiBase}/`);
  ...
  await login(page, `${uiBase}/?releaseJourney=1`, safeCode);
  await page.waitForURL(`${baseURL}${uiBase}/?releaseJourney=1`);
  assert.deepEqual(authRequests.slice(-2), [`${apiBase}/iam/login`, `${apiBase}/iam/mfa`], "browser auth requests escaped the configured API prefix");
  ...
  for (const view of uiViews) {
    await page.goto(`${baseURL}${uiBase}/autopilot/${view}?tender=${tenderId}&company=${companyId}&lot=${lotKey}`, { waitUntil: "networkidle" });
  ...
  await page.goto(`${baseURL}${uiBase}/`, { waitUntil: "networkidle" });
