...
import {documentPipelineStatus,extractSourceLocations,inboxDecision} from "../platform/inbox-pipeline.mjs";
...
test("inbox visibility no longer depends on portal credentials while portal fields remain scoped",()=>{assert.match(routes,/LEFT JOIN LATERAL\(SELECT x\.portal_id,x\.credential_id FROM tender\.current_registered_tender_company_portals x/);assert.match(routes,/x\.tender_id=p\.tender_id AND x\.company_id=p\.company_id/);assert.ok(routes.indexOf("paged AS MATERIALIZED")<routes.indexOf("current_registered_tender_company_portals x"))});
test("portal test jobs use the validated request lot without an undefined body alias",()=>{const route=routes.slice(routes.indexOf('"/api/portal-access/:portalId/jobs"'),routes.indexOf('"/api/management-inbox/autopilot/:tenderId/board-brief"'));assert.match(route,/requireParticipationEligible\(reply,requestedTender,requestedLot\)/);assert.doesNotMatch(route,/\bbody\.lot/)});
