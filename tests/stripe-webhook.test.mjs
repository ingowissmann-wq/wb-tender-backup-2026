...
test("standalone trial charges only 299 for all methods and never creates future charges", async (t) => {
  ...
  for(const [billingPath,method] of [['AUTO_CARD','card'],['INVOICE_BILLIE','billie']]){
  ...
  assert.equal(observed.length,2);assert.ok(observed.every(r=>r.path==='/v1/checkout/sessions'));
});
...
test("all three packages use separate card subscriptions or explicitly paid manual months", async(t)=>{
  ...
  const provider=http.createServer(async(req,res)=>{
    ...
    let body='';for await(const chunk of req)body+=chunk;observed.push(new URLSearchParams(body));res.end(JSON.stringify({id:'cs_package',url:'https://checkout.example.invalid/package'}));
  });provider.listen(0,'127.0.0.1');await once(provider,'listening');t.after(()=>provider.close());
  ...
  for(const plan of Object.keys(prices))for(const billingPath of ['AUTO_CARD','INVOICE_BILLIE']){
    ...
    assert.ok(!session.toString().includes('activation'));
...
test('unknown payment paths are rejected before contacting Stripe',async()=>{
  ...
  await assert.rejects(adapter.createCheckout({purchaseKind:'TRIAL',plan:'TRIAL',billingPath:'UNKNOWN_METHOD',bookingId:TENANT_ID,consentVersion:'standalone-2026-09-07'}),/stripe_billing_path_invalid/);
});
...
test('Billie activates only a signed paid asynchronous event with explicit booking metadata', async t => {
  let activations=0;
  ...
  const event=checkoutEvent({mode:'payment',subscription:null,amount_subtotal:29900,
    metadata:{tenant_id:TENANT_ID,purchase_kind:'TRIAL',plan_code:'TRIAL',billing_path:'INVOICE_BILLIE',booking_id:TENANT_ID,consent_version:'standalone-2026-09-07',renewal:'false'}});
