...
for(const [url,body,field] of routes)test(`${url} rejects invalid, foreign and inactive user assignments before writing`,async()=>{
 for(const target of ['invalid',foreign,actor]){
  ...
  const app=Fastify();registerTenantPortalRoutes(app,{pool,csrf:async()=>{},authenticate:async req=>{req.identity={userId:actor,saas:{tenant_id:tenant,role:'OWNER',plan_code:'PROFESSIONAL',modules:['csm','people'],access:{allowed:true}}};}});
  try{const response=await app.inject({method:'POST',url,payload:{...body,[field]:target}});assert.equal(response.statusCode,400);assert.equal(response.json().message,'active_tenant_member_required');assert.equal(queries.some(q=>q.startsWith('INSERT')),false);assert.ok(queries.includes('ROLLBACK'));}finally{await app.close();}
 }
});
