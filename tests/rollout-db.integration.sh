...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/migrations.log")" -eq 17 ]]
[[ "$(psql "$url" -Atv ON_ERROR_STOP=1 -c "SELECT count(*) FROM tender.release_migrations")" == 17 ]]
...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/production-pending.log")" == 12 ]]
...
printf '{"passed":true,"isolatedPostgres":true,"pendingMigrations":17,"productionPrefixPendingMigrations":12,"runtimeLoginInheritedLeastPrivilege":true,"runtimeSessionsDrainedBeforeRollback":true,"exactReverseRollback":true,"schemaLedgerSnapshotPlansRestored":true,"installedViewAndPriceMetadataPreserved":true}\n'
