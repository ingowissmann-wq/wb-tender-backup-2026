...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/migrations.log")" -eq 18 ]]
[[ "$(psql "$url" -Atv ON_ERROR_STOP=1 -c "SELECT count(*) FROM tender.release_migrations")" == 18 ]]
...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/production-pending.log")" == 13 ]]
...
printf '{"passed":true,"isolatedPostgres":true,"pendingMigrations":18,"productionPrefixPendingMigrations":13,"runtimeLoginInheritedLeastPrivilege":true,"runtimeSessionsDrainedBeforeRollback":true,"exactReverseRollback":true,"schemaLedgerSnapshotPlansRestored":true,"installedViewAndPriceMetadataPreserved":true}\n'
