...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/migrations.log")" -eq 19 ]]
[[ "$(psql "$url" -Atv ON_ERROR_STOP=1 -c "SELECT count(*) FROM tender.release_migrations")" == 19 ]]
...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/production-pending.log")" == 14 ]]
...
printf '{"passed":true,"isolatedPostgres":true,"pendingMigrations":19,"productionPrefixPendingMigrations":14,"runtimeLoginInheritedLeastPrivilege":true,"runtimeSessionsDrainedBeforeRollback":true,"exactReverseRollback":true,"schemaLedgerSnapshotPlansRestored":true,"installedViewAndPriceMetadataPreserved":true}\n'
