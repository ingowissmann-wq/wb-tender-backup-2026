...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/migrations.log")" -eq 14 ]]
[[ "$(psql "$url" -Atv ON_ERROR_STOP=1 -c "SELECT count(*) FROM tender.release_migrations")" == 14 ]]
...
psql "$url" -v ON_ERROR_STOP=1 -f "$root/tests/tenant-company-profiles.integration.sql" >/dev/null
...
sed -n '/^ROLLBACK_MIGRATION=16[0-8]_/p' "$temporary/prefix-seed.log" >"$temporary/prefix-tail.log"
...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/production-pending.log")" == 9 ]]
...
printf '{"passed":true,"isolatedPostgres":true,"pendingMigrations":14,"productionPrefixPendingMigrations":9,"runtimeLoginInheritedLeastPrivilege":true,"runtimeSessionsDrainedBeforeRollback":true,"exactReverseRollback":true,"schemaLedgerSnapshotPlansRestored":true,"installedViewAndPriceMetadataPreserved":true}\n'
