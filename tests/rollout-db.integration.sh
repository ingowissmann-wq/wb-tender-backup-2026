...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/migrations.log")" -eq 21 ]]
[[ "$(psql "$url" -Atv ON_ERROR_STOP=1 -c "SELECT count(*) FROM tender.release_migrations")" == 21 ]]
...
[[ "$(grep -c '^APPLIED_MIGRATION=' "$temporary/production-pending.log")" == 16 ]]
...
printf '{"passed":true,"isolatedPostgres":true,"pendingMigrations":21,"productionPrefixPendingMigrations":16,"runtimeLoginInheritedLeastPrivilege":true,"runtimeSessionsDrainedBeforeRollback":true,"exactReverseRollback":true,"schemaLedgerSnapshotPlansRestored":true,"installedViewAndPriceMetadataPreserved":true}\n'
