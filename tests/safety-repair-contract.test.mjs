...
test("backup automation encrypts, verifies and constrains retention before restore", async () => {
  ...
  assert.match(backup,/DATABASE_CONTAINER/);
  assert.match(backup,/docker exec "\$DATABASE_CONTAINER"/);
  assert.doesNotMatch(backup,/dump_from_container data \| openssl/);
  assert.match(backup,/application\.tar\.zst\.enc/);
  assert.match(backup,/secrets\.tar\.zst\.enc/);
  assert.match(backup,/git -C "\$SOURCE_REPOSITORY" archive --format=tar HEAD/);
  assert.match(backup,/production\/data\/career\.db/);
  assert.match(backup,/secretsSeparated/);
  assert.doesNotMatch(backup,/tar -C "\$PRODUCTION_ROOT" -cf - \./);
  ...
  assert.match(restore,/package_verified=true/);
