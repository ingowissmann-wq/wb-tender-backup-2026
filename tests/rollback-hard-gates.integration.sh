...
mapfile -t down < <(sed -n -e 's/.*-f \(migrations\/158_tender_login_challenge_runtime_grants\.down\.sql\).*/\1/p' -e 's/.*-f deployment\/\(rollback-[^ ]*\.sql\).*/\1/p' -e 's/.*\\i deployment\/\(rollback-[^ ]*\.sql\).*/\1/p' "$temporary/psql.log")
[[ "${down[*]}" == "migrations/158_tender_login_challenge_runtime_grants.down.sql rollback-release-auth-and-commercial-enforcement.sql rollback-approved-tender-commercial-plans.sql rollback-autopilot-overview-latest-lookup.sql" ]]
: >"$temporary/psql.log"
set +e
