...
mapfile -t down < <(sed -n -e 's/.*-f deployment\/\(rollback-[^ ]*\.sql\).*/\1/p' -e 's/.*\\i deployment\/\(rollback-[^ ]*\.sql\).*/\1/p' "$temporary/psql.log")
[[ "${down[*]}" == "rollback-release-auth-and-commercial-enforcement.sql rollback-approved-tender-commercial-plans.sql rollback-autopilot-overview-latest-lookup.sql" ]]
: >"$temporary/psql.log"
set +e
