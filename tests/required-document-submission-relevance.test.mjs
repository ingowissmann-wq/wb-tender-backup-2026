...
function syntheticDb({status="MISSING",mandatory=true,submissionRelevant=true}={}){
  ...
  const pool={async connect(){return client},async query(sql){if(String(sql).includes("SELECT id,source_lifecycle_status,participation_status"))return {rows:[{id:tender,source_lifecycle_status:"ACTIVE",participation_status:"ELIGIBLE",notice_classification:"COMPETITION",offer_deadline:"2099-01-01T00:00:00Z"}]};if(String(sql).includes("FROM tender.tender_lot_lifecycles"))return {rows:[{lot_key:"LOT-0001",lifecycle_status:"ACTIVE",participation_status:"ELIGIBLE",offer_deadline:"2099-01-01T00:00:00Z"}]};if(String(sql).includes("current_tender_company_portal_role_scopes"))return {rows:[{tender_id:tender,company_id:company,source_lot_id:"LOT-0001",portal_role:"SUBMISSION_PORTAL",portal_id:"77777777-7777-4777-8777-777777777777",credential_id:"99999999-9999-4999-8999-999999999999"}]};throw new Error(`unexpected pool query: ${sql}`)}};
