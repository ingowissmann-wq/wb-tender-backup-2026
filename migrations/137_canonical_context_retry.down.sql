...
WITH cancelled AS(
  ...
  SET status='CANCELLED',current_step='MIGRATION_ROLLBACK',
      ...
      result_summary=coalesce(result_summary,'{}'::jsonb)||jsonb_build_object(
        'rollbackRelease','20260825-canonical-context-retry-137.1',
...
SELECT 'canonical_context_retry_rolled_back',jsonb_build_object(
...
DELETE FROM app.schema_migrations WHERE version='0137-canonical-context-retry';
